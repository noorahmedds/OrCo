...
class FSCILTrainer(Trainer):
    ...
    def set_up_model(self):
        self.model = MYNET(self.args, mode=self.args.base_mode)                         # Which final layer is used is determined by the base_mode
        print(MYNET)
    ...
    def train(self):
        ...
        for session in range(args.start_session, args.sessions):
            ...
            self.model = self.update_param(self.model, self.best_model_dict) # Initially self.model is a random model and besty model dict is the pretrained model. Setting the model as the pretrained model while avoiding any layer conflicts.
            ...
            if session == 0:  # load base class train img label
                ...
                for epoch in range(args.epochs_base):
                    ...
                    # >>> Replace only the fully connected layer with the embedding average of the data
                    ...
                    if args.set_no_val: # set no validation
                        ...
                        tsl, tsa, thm = self.test(self.model, testloader, args, session)
                ...
                self.model.module.mode = self.args.new_mode                                                         # The type of classifier used for the incremental session
                self.model.eval()
                trainloader.dataset.transform = testloader.dataset.transform                                        # Carrying over the transforms
                self.model.module.update_fc(trainloader, np.unique(train_set.targets), session)                     # Updating the final classification layer. This includes also training the final layer

                tsl, tsa, thm = self.test(self.model, testloader, args, session)                                         # Testing the model

                # save better model
                self.trlog['max_acc'][session] = float('%.3f' % (tsa * 100))                                        # >>> Saving and logging
                self.trlog['max_hm'][session] = float('%.3f' % (thm * 100))                                        # >>> Saving and logging
                ...
                result_list.append('Session {}, Test Best Epoch {},\nbest test Acc {:.4f}\nbest test hm {:.4f}'.format(
                    session, self.trlog['max_acc_epoch'], self.trlog['max_acc'][session], self.trlog['max_hm'][session]))                          # <<<

        result_list.append(self.trlog['max_acc'])
        print(self.trlog['max_acc'])

        result_list.append(self.trlog['max_hm'])
        print(self.trlog['max_hm'])
    ...
    def validation(self):
        with torch.no_grad():
            ...
            for session in range(1, self.args.sessions):
                ...
                vl, va, vhm = self.test(model, testloader, self.args, session)
    ...
    def base_train(self, model, trainloader, optimizer, scheduler, epoch, args):
        ...
        for i, batch in enumerate(tqdm_gen, 1):
            ...
            # Note data is sampled in such a way that the arg.episode_way/15/n_cls is the number of classes in each batch with arg.episode_shot+arg.epsiode_query/1+15/n_per number of shots

            # >>> Base training using PIL and adaptation module
            k = args.episode_way * args.episode_shot            # Index to the point where the new label data is present
            proto, query = data[:k], data[k:]                   # 1 item goes into proto and 15 go into query as defined earlier
            ...
            # Reshaping from [batch_size, 3, 84, 84] to [epusode_shot, episode_way, 3, 84, 84] and then indexing till low_way
            ...
            proto = proto.view(args.episode_shot, args.episode_way, proto.shape[-1])    # Same as the reshape from earlier
            query = query.view(args.episode_query, args.episode_way, query.shape[-1])

            proto_tmp = proto_tmp.view(args.low_shot, args.low_way, proto.shape[-1])    # Predictions reshaped to match the original input shape
            query_tmp = query_tmp.view(args.episode_query, args.low_way, query.shape[-1])

            proto = proto.mean(0).unsqueeze(0)                                          # Meaned across the 0th axis which is the shot axis
            proto_tmp = proto_tmp.mean(0).unsqueeze(0)                                  # Same here

            proto = torch.cat([proto, proto_tmp], dim=1)                                # Concatenated left to right
