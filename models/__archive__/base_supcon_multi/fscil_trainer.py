    ...
    def train(self):
        ...
        if args.sup_con_pretrain:
            ...
            if args.model_dir is not None:
                ...
                if args.proto_method == "mean":
                    best_prototypes = get_base_fc(base_set, base_testloader.dataset.transform, self.model, args)
                elif args.proto_method == "gm":
                    best_prototypes = get_base_gm(base_set, base_testloader.dataset.transform, self.model, args)
            ...
            if session == 0:  # load base class train img label
                ...
                if self.args.use_gaussian:
                    if gaussian_prototypes is None:
                        ...
                        if self.args.views_gaussian == 1:
                            gaussian_prototypes, prototypeLabels, classVariances, _ = get_prototypes_for_session(base_set, testloader.dataset.transform, self.model, args, 0, use_vector_variance=args.use_vector_variance)
                        else:
                            # Gaussian views use the augmentation from the train loader to get different views
                            # Model the dataset prototypes using the gaussian
                ...
                if not self.args.use_gaussian:
                    if self.args.joint_loading_method == "old":
                        ...
                        joint_set, jointloader, data_previous = get_jointloader(args, session)
                    elif self.args.joint_loading_method == "new":
                        joint_set, jointloader = get_jointloader_fixed(args, session)
                    self.model.module.update_fc_joint(jointloader, testloader, np.unique(joint_set.targets), session, data_previous=data_previous)
                else:
                    ...
                    if self.args.views_gaussian == 1:
                        prototypes_curr, prototypeLabels_curr, classVariances_curr, _ = get_prototypes_for_session(train_set, testloader.dataset.transform, self.model, args, session, use_vector_variance=args.use_vector_variance)
                    else: 
