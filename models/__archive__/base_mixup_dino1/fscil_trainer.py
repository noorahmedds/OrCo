...
class FSCILTrainer(Trainer):
    def __init__(self, args):
        super().__init__(args)
        self.args = args
        self.dino_args = Dinoargs()
        self.dino_transform = DataAugmentationDINO(
            self.dino_args.global_crops_scale,
            self.dino_args.local_crops_scale,
            self.dino_args.local_crops_number,
        )
        try:
        ...
        self.model = MYNET(self.args, self.dino_args, mode=self.args.base_mode, writer = self.writer)
        ...
        if self.args.model_dir is not None:
            ...
            if self.args.start_session > 0:
                # if this is the case then the fc layer has to be replaced as done after the base session
                self.model.module.reset_fc(cuda=True)
    ...
    def get_optimizer_base(self):
        ...
        if self.args.optimizer == "sgd":
            # dino_list = ["module.student", "module.teacher"]
            # base_params = list(filter(lambda kv: not contains(kv[0], dino_list), self.model.named_parameters()))
            # dino_params = list(filter(lambda kv: contains(kv[0], dino_list), self.model.named_parameters()))
            
            if self.args.use_multi_opt:
                base_params = list(self.model.module.encoder.parameters()) + list(self.model.module.fc.parameters())
                dino_params = list(self.model.module.encoder.parameters()) + list(self.model.module.student.parameters())

                cls_optimizer = torch.optim.SGD(base_params, lr=self.args.lr_base, momentum=0.9, nesterov=True, weight_decay=self.args.decay)
                # dino_optimizer = torch.optim.SGD(dino_params, lr=3e-6)
                dino_optimizer = torch.optim.SGD([
                    {"params": self.model.module.student.parameters()},
                    {"params": self.model.module.encoder.parameters(), "lr": 3e-6},
                ], lr=self.args.lr_base)
                
                optimizer = MultipleOptimizer(cls_optimizer, dino_optimizer)
            else:
                # optimizer = torch.optim.SGD(self.model.parameters(), self.args.lr_base, weight_decay=self.args.decay)
                optimizer = torch.optim.SGD(self.model.parameters(), self.args.lr_base, momentum=0.9, nesterov=True,
                                        weight_decay=self.args.decay)

            # TODO: Apply a different rate to the encoder than rest

        elif self.args.optimizer == "adam":
            optimizer = torch.optim.Adam(self.model.parameters(), self.args.lr_base, weight_decay=self.args.decay)
        elif self.args.optimizer == "adamw":
            optimizer = torch.optim.AdamW(self.model.parameters(), self.args.lr_base, weight_decay=self.args.decay)
    ...
    def get_dataloader(self, session):
        if session == 0:
            trainset, trainloader, testloader = get_base_dataloader(self.args, dino_transform = self.dino_transform)
        else:
            trainset, trainloader, testloader = get_new_dataloader(self.args, session, dino_transform=self.dino_transform)
    ...
    def train(self):
        ...
        total_sessions = args.sessions if args.end_session < 0 else args.end_session + 1

        for session in range(args.start_session, total_sessions):
            ...
            if session == 0:  # load base class train img label
                lr_schedule = cosine_scheduler(
                    # args.lr * (args.batch_size_base * 1) / 256.,  # linear scaling rule
                    args.lr_base, # linear scaling rule
                    args.min_lr,
                    args.epochs_base, 
                    len(trainloader),
                ...
                wd_schedule = cosine_scheduler(
                    ...
                    args.epochs_base, len(trainloader),
                )
                ...
                for epoch in range(args.epochs_base):
                    ...
                    if args.s2m2:
                        tl, ta = s2m2_train(self.model, trainloader, optimizer, scheduler, epoch, args, lr_schedule, wd_schedule, momentum_schedule)    
                    else:
                        tl, ta = base_train(self.model, trainloader, optimizer, scheduler, epoch, args, lr_schedule, wd_schedule, momentum_schedule)
                ...
                if not args.not_data_init:
                    ...
                    best_model_dir = os.path.join(args.save_path, 'session' + str(session) + '_max_ft_dot_acc.pth')
                    ...
                    self.model.module.mode = args.replace_mode
                ...
                if session == 1:
                    set_base_avg(train_set, testloader.dataset.transform, self.model, args)
                    print("Base embedding average set")
                ...
                joint_set, jointloader = get_jointloader(args, session, dino_transform=self.dino_transform)
        ...
        hmean = self.trlog['max_hm'][:total_sessions - 1]
        if len(hmean):
            average_harmonic_mean = np.array(hmean).mean()
        else:
