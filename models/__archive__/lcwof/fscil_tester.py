...
class FSCILTester(Trainer):
        ...
        for session in range(1, args.sessions):
            ...
            for epoch in range(args.epochs_novel):
                ...
                if (thm * 100) >= self.trlog['max_hm'][session]:    
                    ...
                    self.trlog['max_acc'][session] = float('%.3f' % (tsa * 100))
                    self.trlog['max_am'][session] = float('%.3f' % (tam * 100))
                    ...
                    print(">>> Found better model: best epoch {}, Harmonic Mean={:.3f}, Top1 Accuracy {:.3f}".format(self.trlog['max_acc_epoch'], self.trlog["max_hm"][session], self.trlog['max_acc'][session])) #  am={:.3f}, hm={:.3f}
            ...
            print('Saving model to :%s' % save_model_dir)
            result_list.append('Session {}, Harmonic Mean={:.3f}, Top1 Accuracy {:.3f}, Arithematic Mean={:.3f}\n'.format(session, self.trlog['max_acc'][session], self.trlog['max_hm'][session], self.trlog['max_am'][session]))
    ...
    def test_novel(self):
        args = self.args
        model = self.update_param(self.model, self.best_model_dict)
        ...
        with torch.no_grad():
            for session in range(1, args.sessions):
                ...
                # result_list.append('Session {}, Harmonic Mean={:.3f}, Top1 Accuracy {:.3f}, Arithematic Mean={:.3f}\n'.format(session, self.trlog['max_acc'][session], self.trlog['max_hm'][session], self.trlog['max_am'][session]))
                output = 'Session {}, Novel Accuracy (Joint Space)={:.3f}, Base Accuracy (Joint Space)={:.3f}, Harmonic Mean={:.3f}, Top1 Accuracy {:.3f}, Arithematic Mean={:.3f}\n'.format(session, va*100, vaNovel*100, vaBase*100, hm(vaNovel, vaBase)*100, am(vaNovel, vaBase)*100)
                print(output)
    ...
    def test(self):
        ...
        novel_results = self.test_novel()
        result_list.extend(novel_results)

        save_list_to_txt(os.path.join(args.save_path, 'results_test.txt'), result_list)
