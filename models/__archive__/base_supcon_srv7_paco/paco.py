...
class PaCoLoss(nn.Module):
    ...
    def forward(self, features, labels=None, sup_logits=None):
        ...
        anchor_dot_contrast = torch.cat(( (sup_logits[:batch_size]) / self.supt, anchor_dot_contrast), dim=1)
        # anchor_dot_contrast = torch.cat(( (sup_logits.T) / self.supt, anchor_dot_contrast), dim=0)
