...
class MYNET(nn.Module):
    ...
    def update_fc_ft_base(self, baseloader, testloader, class_variance=None):
        ...
        paco_criterion = paco.PaCoLoss(num_classes = self.args.base_class if self.args.skip_perturbation else self.fc.reserve_vector_count)
        ...
        with torch.enable_grad():
            ...
            for epoch in tqdm_gen:
                ...
                for idx, batch in enumerate(baseloader):
                    ...
                    # logits_ = self.fc(torch.cat(torch.unbind(features, dim=1), dim=0))
                    # loss = self.args.sup_lam * paco_criterion(features, label_, logits_)
    ...
    def update_fc_ft_joint_supcon(self, jointloader, testloader, class_list, session):
        ...
        sc_criterion = supcon.SupConLoss()
        paco_criterion = paco.PaCoLoss(num_classes = self.args.base_class + (self.args.way * session) if self.args.skip_perturbation_inc else self.fc.reserve_vector_count)
        ...
        with torch.enable_grad():
            ...
            for epoch in tqdm_gen:
                ...
                for batch in jointloader:
                    ...
                    logits_ = self.fc(torch.cat(torch.unbind(features, dim=1), dim=0))
                    logits_rv = torch.matmul(torch.cat(torch.unbind(features, dim=1), dim=0), self.fc.rv.T)
                    logits_ = torch.cat((logits_, logits_rv), axis = 1)
                    sc_loss = self.args.sup_lam * paco_criterion(features, label_, logits_)
