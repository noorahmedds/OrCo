...
class MYNET(nn.Module):
    ...
    def forward_metric(self, x):
        ...
        if 'cos' in self.mode:
            ...
            x = self.fc(x)
        elif 'l2' in self.args.new_mode:
            l2 = torch.cdist(x, self.fc.weight)     # Each row here represents the distance of that sample from all the hundred classes
            x = self.fc(x)
            x += 1/l2
    ...
    def get_logits(self,x,fc):
        if 'dot' in self.args.new_mode:
            ...
            l2 = torch.cdist(x, fc)     # Each row here represents the distance of that sample from all the hundred classes
            x = F.linear(x, fc)
            x += 1/l2
            return x
        ...
        with torch.enable_grad():
            ...
            for epoch in tqdm_gen:
                ...
                for data, label in trainloader:
                    ...
                    # Computations to ensure only current novel classes and base classes are taken into account
                    # >> Taking logits only belonging to the current novel class and the base classes
                    # wanted_cols = list(range(0, self.args.base_class)) + \
                    #             list(range(self.args.base_class + self.args.way * (session - 1), self.args.base_class + self.args.way * session))
                    # label = label - label_offset
                    # logits = logits[:, wanted_cols]
