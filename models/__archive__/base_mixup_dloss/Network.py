...
# from data_utils import get_previous_novel_dataloader
...
class MYNET(nn.Module):
    ...
    def __init__(self, args, mode=None, writer=None):
        ...
        self.previous_fc = None
        # with torch.no_grad(): self.previous_fc.weight[:self.args.base_class + self.args.way * session, :] = self.fc.weight.detach()
    ...
    def update_fc_ft_novel(self, new_fc, trainloader, testloader, session):
        ...
        # concat_fc = nn.Parameter(torch.cat([old_fc, new_fc], dim = 0))
        optimized_parameters = [{'params': new_fc}] # Training the entire fc
        ...
        # # TEST: Get a loader for all the previously seen novel classes
        # prev_trainset, prev_trainloader, prev_testloader = get_previous_novel_dataloader(args, session)
            ...
            for epoch in tqdm_gen:
                ...
                for data, label in trainloader:
                    ...
                    if self.args.mixup_novel:
                        ...
                        # Note my undestanding of the distillation loss is a little flawed.
                        # So for Di we take a sample and pass it through our two networks. And get the softmax.
                        # Instead of the previous ones.
                        # In the loader sample the Di data. i.e. the new data that belongs to the current novel classes.
                        # Create a copy of the fc layer (trained up until before this session)
                        # Pass the entire fc into another optimiser
                        # Now for this data get the logits fro both the fc layers. 
                        # Apply softmax on both only uptil the the previous session classes. So not on the entired space but just the previous sub space
                        # Then take the log product of the two probabilities
                        # And pass to your optimiser along with the class loss.
                        if self.previous_fc is not None:
                            import pdb; pdb.set_trace()
                            
                        #     # # Sample an image from the previous class and see
                        #     # data, label = next(iter(prev_trainloader))
                        #     # data = data.cuda(); label = label.cuda()

                        #     # prev_encoding = self.encode(data).detach() #<<< The encoder is essentially frozen

                        #     # logits_new = self.get_logits(prev_encoding, concat_fc)
                        #     # logits_old = self.get_logits(prev_encoding, self.previous_fc)

                        #     # probs_new = F.softmax(logits_new)
                        #     # probs_old = F.softmax(logits_old)

                        #     # d_loss = -(torch.sum(probs_old * torch.log(probs_new)))
                        #     # loss += d_loss

                        #     encoding = self.encode(data).detach()
                        #     logits_new = self.get_logits(encoding, concat_fc[:, :certain_point])
                        #     logits_old = self.get_logits(encoding, self.previous_fc[:, :certain_point])
        ...
        self.previous_fc = self.fc.weight.detach().clone()
