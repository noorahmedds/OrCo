...
class MYNET(nn.Module):
    ...
    def get_optimizer_new(self, optimized_parameters):
        ...
        elif self.args.optimizer == "mtadam":
            optimizer = MTAdam(optimized_parameters, lr=self.args.lr_new, weight_decay=self.args.decay_new)  # wd:1e-1
        return optimizer
    ...
    def get_optimizer_base(self, optimized_parameters):
        ...
        if self.args.optimizer == "sgd":
            ...
            optimizer = MTAdam(optimized_parameters, lr=self.args.lr_base, weight_decay=self.args.decay)  # wd:1e-1
        ...
        with torch.enable_grad():
            ...
            for epoch in tqdm_gen:
                ...
                for idx, batch in enumerate(baseloader):
                    ...
                    loss_array.append(sc_criterion(features, label_))
                    ...
                    if not self.args.spread_aware:
                        ...
                        label_rep = label.repeat(2)
                        # loss += self.args.cos_lam * self.criterion_forward(criterion, logits, label_rep, base_sess = True)
                        loss_array.append(self.criterion_forward(criterion, logits, label_rep, base_sess = True))
                    else:
