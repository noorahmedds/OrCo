...
class MYNET(nn.Module):
    ...
    def __init__(self, args, mode=None, writer=None):
        ...
        # self.params_unit_update = []
        # self.unit_optimizer_params = [{"params": self.parameters()}]    # When you would like to optimise all the parameters when training the unit projection module
        self.unit_optimizer_params = [{"params": self.encoder.layer4.parameters()}, {"params": self.unit_projection.parameters()}]   # # When you would like to optimise only the last layer of the encoder and the unit projection module
    ...
    def get_optimizer_new(self, optimized_parameters, lr = None):
        if self.args.optimizer == "sgd":
            optimizer = torch.optim.SGD(optimized_parameters,lr=self.args.lr_new if lr is None else lr, momentum=0.9, dampening=0.9, weight_decay=self.args.decay_new)
    ...
    def update_fc_ft_novel(self, new_fc, trainloader, testloader, session):
        ...
        cls_optimizer = self.get_optimizer_new([{"params":new_fc}, {"params":self.cls_projection.parameters()}])
        unit_optimizer = self.get_optimizer_new(self.unit_optimizer_params, lr = 3e-4)
        ...
        with torch.enable_grad():
            ...
            for epoch in tqdm_gen:
                ...
                for data, label in trainloader:
                    ...
                    encoding = self.encode(data) # <<< The encoder is essentially frozen
                    ...
                    if self.args.mixup_novel:
                        cls_loss = mixup_criterion(F.cross_entropy, cls_out, targets_a, targets_b, lam)
                    else:
                        # The loss now in the denominator accounts for all the novel classes + base classes
                        cls_loss = F.cross_entropy(cls_out, label) # Note, no label smoothing here

                    projection_loss = F.cross_entropy(proj_out, label)                    
                    ta.add(count_acc(cls_out, label))

                    loss = cls_loss + projection_loss
                    optimizer.zero_grad()
                    loss.backward()
                    optimizer.step()

                    total_loss += cls_loss.item() + projection_loss.item()
    ...
    def update_fc_ft_joint(self, jointloader, testloader, class_list, session):
        ...
        cls_optimizer = self.get_optimizer_new([{"params":[base_fc, new_fc]}, {"params":self.cls_projection.parameters()}])
        unit_optimizer = self.get_optimizer_new(self.unit_optimizer_params, lr = 3e-4)
        optimizer = MultipleOptimizer(cls_optimizer, unit_optimizer)
            ...
            for epoch in tqdm_gen:
                ...
                for data, label in jointloader:
                    ...
                    if self.args.mixup_joint:
                        ...
                        # The cls_loss now in the denominator accounts for all the novel classes + base classes
                        cls_loss = F.cross_entropy(cls_out, label) # Note, no label smoothing here
