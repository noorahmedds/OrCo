...
def test_fixed(model, testloader, epoch, args, session):
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(tqdm_gen, 1):
            ...
            logits, projections = model(data)
            logits = logits[:, :test_class]

            projection_features.append(projections)
    ...
    projection_features = torch.cat(projection_features, axis = 0)
    cos_sims["inter"] = compute_inter_class_cosine_similarity(None, None, projection_features, all_targets)
    cos_sims["intra"] = compute_intra_class_cosine_similarity(None, None, projection_features, all_targets)
...
def extract_features(model, loader):
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(tqdm_gen, 1):
            data, label = [_.cuda() for _ in batch]
            projections, encodings = model.module.encode(data, return_encodings=True)
            backbone_features.append(encodings.detach())
            projection_features.append(projections.detach())
            labels.append(label)

    backbone_features = torch.cat(backbone_features, axis = 0)
    projection_features = torch.cat(projection_features, axis = 0)
    # features = F.normalize(features, p=2, dim=-1)

    labels = torch.cat(labels)
    return backbone_features, projection_features, labels

def compute_intra_class_cosine_similarity(model, testloader, projection_features=None, test_targets=None):
    if projection_features is not None:
        test_projection_features = normalize(projection_features)
    else:    
        model = model.eval()
        test_backbone_features, test_projection_features, test_targets = extract_features(model, testloader)
        test_projection_features = normalize(test_projection_features)
...
def compute_inter_class_cosine_similarity(model, testloader, projection_features=None, test_targets=None):
    if projection_features is not None:
        test_projection_features = normalize(projection_features)
    else:    
        model = model.eval()
...
def test_knn(model, trainset, testloader, k=5):
    model = model.eval()
    ...
    train_backbone_features, train_projection_features, train_targets = extract_features(model, trainloader)
    test_backbone_features, test_projection_features, test_targets = extract_features(model, testloader)
    ...
    acc1_projection = []
    for k_i in k:
        knn_backbone = WeightedKNNClassifier(
            k=k_i,
            distance_fx="cosine",
        )
        
        knn_projection = WeightedKNNClassifier(
            k=k_i,
            distance_fx="cosine",
        )
        
        knn_backbone(
            train_features=train_backbone_features,
            train_targets=train_targets,
            test_features=test_backbone_features,
            test_targets=test_targets,
        )
        
        knn_projection(
            train_features=train_projection_features,
            train_targets=train_targets,
            test_features=test_projection_features,
            test_targets=test_targets,
        )
    
        acc1, _ = knn_backbone.compute()
        acc1_backbone.append(acc1)
        
        acc1, _ = knn_projection.compute()
        acc1_projection.append(acc1)
    
        del knn_backbone, knn_projection
    
    return np.array(acc1_backbone).mean(), np.array(acc1_projection).mean()