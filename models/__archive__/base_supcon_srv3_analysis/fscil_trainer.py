...
class FSCILTrainer(Trainer):
    ...
    def train(self):
        ...
        if args.sup_con_pretrain:
            ...
            if args.model_dir is not None:
                ...
                if can_resume:
                    ...
                    # mean_knn_acc1_backbone, mean_knn_acc1_projection,  = test_knn(self.model, incset, inc_testloader, k=[2,3,4,5])
                    # print(f"==>Pretrain 1st Incremental Session KNN-Score {mean_knn_acc1_backbone:.3f}/{mean_knn_acc1_projection:.3f}")

                    # incset, _, inc_testloader = self.get_dataloader(1)
                    # avg_cosine_sim = compute_intra_class_cosine_similarity(self.model, inc_testloader)
                    # avg_inter_cosine_sim = compute_inter_class_cosine_similarity(self.model, inc_testloader)
                    # print(f"==>[Pretrain] Average Intra/Inter Class Cosine Similarity: {avg_cosine_sim:.3f}/{avg_inter_cosine_sim:.3f}")
            ...
            # Compute the intra class cosine similarity after the prep session
            # incset, inc_trainloader, inc_testloader = self.get_dataloader(1)
            # avg_cosine_sim = compute_intra_class_cosine_similarity(self.model, inc_testloader)
            # avg_inter_cosine_sim = compute_inter_class_cosine_similarity(self.model, inc_testloader)
            # print(f"==>[Preparatory] Average Intra/Inter Class Cosine Similarity: {avg_cosine_sim:.3f}/{avg_inter_cosine_sim:.3f}") 
            ...
            incset, _, inc_testloader = self.get_dataloader(1)
            mean_knn_acc1_backbone, mean_knn_acc1_projection,  = test_knn(self.model, incset, inc_testloader, k=[2,3,4,5])
            print(f"==>Preparatory 1st Incremental Session KNN-Score {mean_knn_acc1_backbone:.3f}/{mean_knn_acc1_projection:.3f}")

            # Print KNN Scores
            print(f"===[Pretrain] KNN Accuracy: {pretrain_knn_acc1:.3f}")
            print(f"===[Preparatory] KNN Accuracy: {prep_knn_acc1:.3f}")
            print(f"===[Preparatory] 1st Sessions Class KNN score: {mean_knn_acc1_projection:.3f}")
            print(f"===[Preparatory] Class Wise Accuracy: {cw_acc*100:.3f}")
            print(f"===[Preparatory] Accuracy: {best_va*100:.3f}")
            print(f"===[Preparatory] Cosine Similarity: {cos_sims}")
        ...
        for session in range(args.start_session, args.end_session):
            ...
            tsl, tsa, tsaNovel, tsaBase, vaSession, cw_acc, novel_cw, base_cw, cos_sims = test_fixed(self.model, testloader, 0, args, session)
            ...
            out_string = 'Session {}, test Acc {:.3f}, test_novel_acc {:.3f}, test_base_acc {:.3f}, \
                hm {:.3f}, am {:.3f}, cw_acc {:.3f}, hm_cw {:.3f}, cos_sims {}'\
                .format(
                    session, 
