...
#         Args:
#             features: hidden vector of shape [bsz, n_views, ...]. Note the features are normalised. This will save the model from very large exponent values
...
#         # Remove samples that don't have another unique sample from the same class
#         uni_labels, uni_count = labels.unique(return_counts = True)
#         uni_labels = uni_labels[uni_count > 1]
#         filter_ix = []
#         for ix, l in enumerate(labels):
#             if l in uni_labels:
#                 filter_ix.append(ix)
#         features = features[filter_ix]
#         labels = labels[filter_ix]
...
#             mask = torch.eq(labels, labels.T).float().to(device) # mask defined
#         else:
#             mask = mask.float().to(device)

#         contrast_count = features.shape[1]
#         contrast_feature = torch.cat(torch.unbind(features, dim=1), dim=0) # reduce dimensions for the views into 
#         if self.contrast_mode == 'one':
#             # Using a single anchor for the loss computation
#             anchor_feature = features[:, 0]
#             anchor_count = 1
#         elif self.contrast_mode == 'all':
#             # Computing the loss over all samples by using each sample as an anchor
...
#         # compute log_prob
#         exp_logits = torch.exp(logits) * logits_mask # masking self contrast and same class cases. If we have negative values the exponent produces 0 values
#         log_prob = logits - torch.log(exp_logits.sum(1, keepdim=True)) # Now if the exponenent is 0 or very close to 0 the log is undefined. 
...
class SupConLoss(nn.Module):
    ...
    def forward(self, features, labels=None, mask=None, margin = 0):
        ...
        # compute logits
        anchor_dot_contrast = torch.div(
            torch.matmul(anchor_feature, contrast_feature.T),   # Inner product (in contrast mode all: the inner product is between all features)
            self.temperature)
        ...
        mask = mask * logits_mask   # in the original mask we now suppress self contrast cases
...
def find_eq_vector(vector_as_rows):
    ...
    #     dists.append(torch.linalg.norm(v - res))
...
def find_equidistant_vectors(vectors):
