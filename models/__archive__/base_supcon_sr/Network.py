...
class SRHead(nn.Module):
    ...
    def assign_base_classifier(self, base_prototypes, base_cov=None):
        ...
        if self.args.assign_similarity_metric == "cos":
            ...
            cost = compute_pairwise_mahalanobis(base_prototypes.cpu(), base_cov, self.rv.cpu())
        # row_ind, col_ind = linear_sum_assignment(cost, maximize = True)
...
class MYNET(nn.Module):
    ...
    def __init__(self, args, mode=None, writer=None):
        ...
        self.base_projection_head_copy = None
    ...
    def get_optimizer_new(self, optimized_parameters):
        if self.args.optimizer == "sgd":
            ...
            optimizer = torch.optim.AdamW(optimized_parameters, lr=self.args.lr_new, weight_decay=self.args.decay_new)
        return optimizer
    ...
    def criterion_forward(self, criterion, logits, label, cos_margin = 0):
        if self.args.criterion == "xent":
            ...
            if cos_margin > 0:
                logits = logits - (one_hot_label * cos_margin)
    ...
    def update_fc_ft_base(self, baseloader, testloader):
        ...
        with torch.enable_grad():
            ...
            for epoch in tqdm_gen:
                ...
                for idx, batch in enumerate(baseloader):
                    ...
                    loss = self.criterion_forward(criterion, logits, label, cos_margin = self.args.cos_b_margin)
        ...
        print(f"Weight shift required was = {compute_parameter_diff(theta1, self.projector):.3f}")
    ...
    def pull_loss(self, label_rep, novel_class_start, criterion, logits, tl, session):
        if self.args.pull_loss_mode == "default":
            ...
            if novel_classes_idx.numel() != 0:
                novel_loss = self.criterion_forward(criterion, logits[novel_classes_idx, :], label_rep[novel_classes_idx], cos_margin = self.args.cos_n_margin)                            
                tl["novel"].add(novel_loss.item())
            if base_classes_idx.numel() != 0:
                base_loss = self.criterion_forward(criterion, logits[base_classes_idx, :], label_rep[base_classes_idx], cos_margin = self.args.cos_n_margin)
                tl["base"].add(base_loss.item())
    ...
    def update_fc_ft_joint_supcon(self, jointloader, testloader, class_list, session):
        if self.args.init_sess_w_base_proj:
            self.projector.load_state_dict(self.base_projection_head_copy, strict=True)
        ...
        with torch.enable_grad():
            ...
            for epoch in tqdm_gen:
                ...
                for batch in jointloader:
                    ...
                    if self.args.supcon_even:
                        _label = label.repeat(2)
                        novel_classes_idx = torch.argwhere(_label >= novel_class_start).flatten()
                        base_classes_idx = torch.argwhere(_label < novel_class_start).flatten()
                        novel_label = label[torch.argwhere(label >= novel_class_start).flatten()]
                        base_label = label[torch.argwhere(label < novel_class_start).flatten()]
                        novel_projections = projections[novel_classes_idx]
                        base_projections = projections[base_classes_idx]
                        # Filter projections by novel and not novel
                        novel_sc_loss = 0
                        if novel_classes_idx.numel() != 0:
                            f1, f2 = torch.split(novel_projections, [novel_label.shape[0], novel_label.shape[0]], dim=0)
                            features = torch.cat([f1.unsqueeze(1), f2.unsqueeze(1)], dim=1)
                            novel_sc_loss = sc_criterion(features, novel_label, target_prototypes = target_prototypes)

                        base_sc_loss = 0
                        if base_classes_idx.numel() != 0:
                            f1, f2 = torch.split(base_projections, [base_label.shape[0], base_label.shape[0]], dim=0)
                            features = torch.cat([f1.unsqueeze(1), f2.unsqueeze(1)], dim=1)
                            base_sc_loss = sc_criterion(features, base_label, target_prototypes = target_prototypes)

                        sc_loss = 0.5*novel_sc_loss + (1-0.5)*base_sc_loss
                    else:
                        f1, f2 = torch.split(projections, [bsz, bsz], dim=0)
                        features = torch.cat([f1.unsqueeze(1), f2.unsqueeze(1)], dim=1)
                        sc_loss = sc_criterion(features, label, target_prototypes = target_prototypes)

                    logits = self.fc(projections)
                    label_rep = label.repeat(2)
                    cos_loss = self.pull_loss(label_rep, novel_class_start, criterion, logits, tl, session)

                    ta.add(count_acc(logits, label_rep))

                    # Remove cosine loss in the case we are using the sup con pull method
                    if self.args.lam_at > 0:
                        cos_loss = 0

                    # loss = sc_loss + cos_loss
                    loss = (self.args.sup_lam*sc_loss) + (self.args.cos_lam*cos_loss)

                    if self.args.compute_hardnegative:
                        loss += 1e-3*compute_hn_cosine_embedding_loss(projections, logits, label_rep, session)
                    ...
                    if self.args.proj_ema_update:
                        # prev_weight = self.projector_ema.ema_model[0].weight.clone()
                        self.projector_ema.update()
                        # after_weight = self.projector_ema.ema_model[0].weight
                        # print(f"Sum: {(after_weight-prev_weight).sum():.5f}")
