        ...
        if args.sup_con_pretrain:
            ...
            if args.model_dir is not None:
                ...
                if can_resume:
                    ...
                    if self.args.init_proj_random:
                        # Replace the projector head to start afresh. The projector will now have to learn everything from scratch
                        self.model.module.init_proj_random()

                    # Get mean prototypes from the projection head
                    best_prototypes = get_base_fc(base_set, base_testloader.dataset.transform, self.model, args, return_cov = self.args.assign_similarity_metric == "mahalanobis")

                    base_cov = None
                    if self.args.assign_similarity_metric == "mahalanobis":
                        best_prototypes, base_cov = best_prototypes

                    # Compute validation score using prototypes
                    ...
                    # Optmise the reserve vectors using the base classifier. Set the number of reserve vectors as the number of incoming classes.
                    # In the case of minet only 40
                    if args.reserve_mode == "novel":
                        self.model.module.fc.find_reseverve_vectors()
                        
                        # best_va = self.model.module.update_fc_ft_base(base_trainloader, base_testloader)
                    elif args.reserve_mode == "all":
                        self.model.module.fc.find_reseverve_vectors_all()

                        # Assign classifier to the best prototypes
                        self.model.module.fc.assign_base_classifier(best_prototypes, base_cov=base_cov)
                        ...
                        best_va = self.model.module.update_fc_ft_base(base_trainloader, base_testloader)
                    elif args.reserve_mode == "two_step":
                        self.model.module.fc.find_reseverve_vectors_two_step(best_prototypes)
                        ...
                        # Fine tune projection unit, update the fc.classifiers[0] to the new prototypes
                        best_va = self.model.module.update_fc_ft_base(base_trainloader, base_testloader)
                    elif args.reserve_mode == "base_init":
                        # Initialise the points with base prototypes
                        self.model.module.fc.find_reseverve_vectors_base_init(best_prototypes)
            ...
            if self.args.append_hard_positives:
                print("===Saving hard positive exemplars")
                self.model.module.get_hard_base_classes(base_set, base_trainloader)

        if args.init_sess_w_base_proj:
            self.model.module.base_projection_head_copy = deepcopy(self.model.module.projector.state_dict())
        ...
        for session in range(args.start_session, args.end_session):
            ...
            if not self.args.use_gaussian:
                if self.args.joint_supcon:
                    ...
                    joint_set, jointloader = get_jointloader_fixed(args, session)
                ...
                if gaussian_prototypes is None:
                    ...
                    # gaussian_prototypes, prototypeLabels, classVariances, _ = get_prototypes_for_session(base_set, trainloader.dataset.transform, self.model, args, 0, use_vector_variance=args.use_vector_variance, views=self.args.views_gaussian)
                    gaussian_prototypes, prototypeLabels, classVariances, _ = get_prototypes_for_session(base_set, self.strong_transform, self.model, args, 0, use_vector_variance=args.use_vector_variance, views=self.args.views_gaussian)

                if self.args.use_ncfscil: 
                    # Apply ncfscil using the gaussian model
                    self.model.module.update_fc_ft_joint_nc_fscil(trainloader, testloader, session, gaussian_prototypes, prototypeLabels)    
                else:
                    # generated = generateMutivariateData(gaussian_prototypes, prototypeLabels, classVariances, args.shot)
                    generated = generateMutivariateData(gaussian_prototypes, prototypeLabels, classVariances, 1)
                    generated_aug = generateMutivariateData(gaussian_prototypes, prototypeLabels, classVariances, 200) # Other gaussian generated vectors. 200 per class which are then used to augment the training data
                    
                    # Create a dataset wrapper where the generated features and train set are all combined
                    if self.args.joint_loading_method == "old":
                        jointloader = torch.utils.data.DataLoader(dataset=train_set, batch_size=args.batch_size_joint, shuffle=True,
                                                    num_workers=args.num_workers, pin_memory=True)
                        self.model.module.update_fc_joint(jointloader, testloader, np.unique(train_set.targets), session, generated=(generated, generated_aug))                         
                    elif self.args.joint_loading_method == "new":
                        gaussian_data, gaussian_labels = generated
                        aug_gaus_data, aug_gaus_labels = generated_aug
                        jointset = GaussianWrapperDataSet(train_set, gaussian_data, gaussian_labels, aug_gaus_data, aug_gaus_labels)
                        jointloader = torch.utils.data.DataLoader(dataset=jointset, batch_size=args.batch_size_joint, shuffle=True,
                                                                num_workers=args.num_workers, pin_memory=True)
                        self.model.module.update_fc_ft_joint_gaussian_loader(jointloader, testloader, np.unique(train_set.targets), session)
                    elif self.args.joint_loading_method == "online":
                        # Sample the gaussian data within the training loop for more variety
                        self.model.module.update_fc_ft_joint_online(trainloader, testloader, session, gaussian_prototypes, prototypeLabels, classVariances)

                # Compute prototypes and update 
                # prototypes_curr, prototypeLabels_curr, classVariances_curr, _ = get_prototypes_for_session(train_set, testloader.dataset.transform, self.model, args, session, use_vector_variance=args.use_vector_variance, views=args.views_gaussian)
                prototypes_curr, prototypeLabels_curr, classVariances_curr, _ = get_prototypes_for_session(train_set, self.strong_transform, self.model, args, session, use_vector_variance=args.use_vector_variance, views=args.views_gaussian)
            ...
            # tsl, tsa, tsaNovel, tsaBase, tsHm, tsAm, tcm, tcmNovel, tcmSummary, tsaPrevNovel, vaSession = test(self.model, testloader, 0, args, session)        # Perform the validation inside the classifier updates and save the best mode    
