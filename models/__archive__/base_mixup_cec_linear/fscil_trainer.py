        ...
        for session in range(args.start_session, args.sessions):
            ...
            if session == 0:  # load base class train img label
                ...
                for epoch in range(args.epochs_base):
                    ...
                    tl, ta = cec_simple_base_train(self.model, trainloader, optimizer, scheduler, epoch, args)
                    
                    # Replace the gully connected layer every epoch for testing purposes
                    self.model = replace_base_fc(train_set, testloader.dataset.transform, self.model, args)

                    # test model with all seen class
                    # tsl, tsa = (test(self.model, testloader, epoch, args, session, base_sess = True))[:2]
                    tsl, tsa, thm = cec_simple_test(self.model, testloader, args, session)
