...
class MYNET(nn.Module):
    ...
    def _forward(self, support, query):
        ...
        # Mean across the number of shots
        proto = support.mean(dim=1) # Each class now has a mean prototype
        num_batch = proto.shape[0]
        num_proto = proto.shape[1]
        num_query = query.shape[1] * query.shape[2]#num of query*way #for example [1,10,30,512] will have 300.

        # query: (num_batch, num_query, num_proto, num_emb)
        # proto: (num_batch, num_proto, num_emb)
        # NEW
        query = query.view(-1, emb_dim)    # Collapse the batches, shots and way axis into one
        proto = proto.squeeze(0)

        # ORIGINAL CEC
        # query = query.view(-1, emb_dim).unsqueeze(1)    # Collapse the batches, shots and way axis into one. and add a new dim
        ...
        # logits=F.cosine_similarity(query,proto,dim=-1) # <====== Original
        logits = F.linear(query, proto) # This should tell the meta learning scheme to adjust to a dot product relation between the 
