...
def cec_simple_base_train(model, trainloader, optimizer, scheduler, epoch, args):
    tl = Averager()
    ta = Averager()
    ...
    for i, batch in enumerate(tqdm_gen, 1):
        ...
        # Calculating the prototypes here
        proto = proto.mean(0)          # Prototypes generated for the actual way shot setting with the non rotated latent vectors
        emb_dim = proto.size(-1)

        # Collapsing the queries for matching
        query = query.view(-1, emb_dim)

        logits = F.linear(query, proto)
        total_loss = F.cross_entropy(logits, label)
...
def cec_simple_test(model, testloader, args, session):
