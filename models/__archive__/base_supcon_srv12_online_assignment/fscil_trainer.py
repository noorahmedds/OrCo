...
class FSCILTrainer(Trainer):
    ...
    def train(self):
        ...
        for session in range(args.start_session, args.end_session):
            ...
            self.model.module.update_fc_ft_joint_supcon(jointloader, testloader, np.unique(joint_set.targets), session, _joint_set, _jointloader)

            # Compute accuracy and KNN score for pretrain session
            tsl, tsa, tsaNovel, tsaBase, vaSession, cw_acc, novel_cw, base_cw = test_fixed(self.model, testloader, 0, args, session)
            ...
            out_string = 'Session {}, test Acc {:.3f}, test_novel_acc {:.3f}, test_base_acc {:.3f}, hm {:.3f}, am {:.3f}, cw_acc {:.3f}, hm_cw {:.3f}'\
                .format(
                    session, 
