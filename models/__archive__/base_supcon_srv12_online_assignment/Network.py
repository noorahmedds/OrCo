...
class SRHead(nn.Module):
    ...
    def get_assignment(self, cost, assignment_mode, prototypes):
        ...
        if assignment_mode == "max":
            ...
            row_ind, col_ind = linear_sum_assignment(cost, maximize = True)
        print("Cost mean sum: ", cost[row_ind, col_ind].mean())
        return col_ind
    ...
    def assign_base_classifier(self, base_prototypes, base_cov=None):
        ...
        if not self.args.online_assignment:
            ...
            all_idx = np.arange(self.rv.shape[0])
            self.unassigned_rv = self.rv[all_idx[~np.isin(all_idx, col_ind)]]
            self.inc_rv = self.rv[all_idx[~np.isin(all_idx, col_ind)]]
    ...
    def assign_novel_classifier(self, new_prototypes, online = False, cov_list = None):  
        ...
        if self.args.assign_similarity_metric == "cos":
            ...
            # TODO : Compute cost of prototype to already assigned classifiers
            # If the similarity is high for an already assigned classifier then penalise high cosine similarity
            # By computing reciprocal of cosine similarity between that prototype and rv. i.e. the row which contains scores for all pairs containing
            # the confusing new_prototype is reciprocal.
            assigned_targets = self.get_classifier_weights()
            sub_cost = cosine_similarity(new_prototypes.cpu(), assigned_targets.cpu())

            for i in range(sub_cost.shape[0]):
                if sub_cost[i].max() > cost[i].max():   # If the current cost is not max then we assume a reciprocal
                    cost[i] = 1/cost[i]
    ...
    def assign_online(self, prototypes):
        ...
        if self.args.assign_similarity_metric == "cos":
            cost = cosine_similarity(prototypes.cpu(), self.inc_rv.cpu())
        elif self.args.assign_similarity_metric == "euclidean":
            cost = euclidean_distances(prototypes.cpu(), self.inc_rv.cpu())

        # Inherit assignment mode from base
        assignment_mode = self.args.assignment_mode_novel
        col_ind = self.get_assignment(cost, assignment_mode, prototypes)
        
        fc_tensor = self.inc_rv[col_ind]
        ...
        for i, cls in enumerate(self.classifiers.children()):
            if i == 0:
                ...
                n_class_start = self.args.way * (i-1)
                n_class_end = self.args.way * i
                cls.weight.data = fc_tensor[n_class_start:n_class_end]

        # And rv remains un altered
        all_idx = np.arange(self.inc_rv.shape[0])
        self.unassigned_rv = self.inc_rv[all_idx[~np.isin(all_idx, col_ind)]]
...
class MYNET(nn.Module):
    ...
    def update_fc(self, trainloader, testloader, class_list, session, mode="encoder"):
        if not self.args.online_assignment: # Skip assignment if online assignment
            ...
            new_prototypes, cov_list = self.get_class_avg(trainloader, class_list)
    ...
    def get_class_avg(self, trainloader, class_list):
        ...
        for class_index in class_list:
            data_index=(all_label==class_index).nonzero().squeeze(-1)
            embedding=all_data[data_index]
        ...
        online=torch.stack(new_fc,dim=0)

        return online, cov_list
    ...
    def update_fc_ft_base(self, baseloader, testloader, class_variance=None):
        ...
        if self.args.perturb_mode_base == "inc":
            # Perturb only the targets which are unassigned
            target_prototypes = torch.nn.Parameter(self.fc.unassigned_rv)
        ...
        elif self.args.perturb_mode_base == "all":
            target_prototypes = torch.nn.Parameter(torch.cat((self.fc.get_classifier_weights(), self.fc.unassigned_rv), axis = 0))
            ...
            for epoch in tqdm_gen:
                ...
                for idx, batch in enumerate(baseloader):
                    ...
                    if self.args.skip_perturbation:
                        ...
                        pert, target_labels_ = perturb_targets_norm_count(target_prototypes.clone(), target_labels.clone(), bsz, 2, epsilon = self.args.perturb_epsilon_base)
                ...
                # TODO: Get prototypes, do new assignment
    ...
    def update_fc_ft_joint_supcon(self, jointloader, testloader, class_list, session, _joint_set, _jointloader):
        # Optimising only the projector
        ...
        # Create a new classifier for the novel sessions
        # Online assign the sup con
        
        # optimizer_geom = self.get_optimizer_new(self.projector.parameters())
        with torch.enable_grad():
            ...
            for epoch in tqdm_gen:
                for batch in jointloader:
                    ...
                    sloss = simplex_loss_in_batch(f1)
                    # sloss = simplex_loss(f1[new_ixs], label, assigned_targets, assigned_targets_label, unassigned_targets)

                    optimizer.zero_grad()
                    sloss.backward()
                    optimizer.step()
                tqdm_gen.set_description(f"Geometric Loss: {sloss.item():.3f}")

                    
        if self.args.online_assignment:
            self.fc.create_new_classifier()
            # online, cov = self.get_class_avg(_jointloader, np.unique(_joint_set.targets))
            online, cov = self.get_class_avg(_jointloader, np.arange(self.args.way * session) + self.args.base_class)
            self.fc.assign_online(online)
            # Here create the perturb targets

            # target_prototypes = torch.nn.Parameter(torch.cat((self.fc.get_classifier_weights(uptil=session)[self.args.base_class:].clone(), self.fc.unassigned_rv.clone())))
            # target_prototypes.requires_grad = False
            # # target_labels = torch.arange(self.args.num_classes - self.args.base_class).cuda() + self.args.base_class
            # target_labels = torch.arange(self.fc.reserve_vector_count - self.args.base_class).cuda() + self.args.base_class
            ...
            if self.args.perturb_mode_inc == "inc-curr-base": #"all", "inc+curr-base", "inc-curr+base"
                ...
                mask = ~torch.isin(target_labels, curr_labels)
                target_labels = target_labels[mask]
            elif self.args.perturb_mode_inc == "inc+curr-base":
                # Target prototypes contains assigned targets and unassigned targets (except base)
                target_prototypes = torch.nn.Parameter(torch.cat((self.fc.get_classifier_weights(uptil=session+1)[self.args.base_class:].clone(), self.fc.unassigned_rv.clone())))
                target_prototypes.requires_grad = False
                target_labels = torch.arange(self.fc.reserve_vector_count - self.args.base_class).cuda() + self.args.base_class
            elif self.args.perturb_mode_inc == "inc-curr+base":
                # Target prototypes contains assigned targets and unassigned targets (except base)
                target_prototypes = torch.nn.Parameter(torch.cat((self.fc.get_classifier_weights(uptil=session).clone(), self.fc.unassigned_rv.clone())))
                target_prototypes.requires_grad = False
                target_labels = torch.arange(self.fc.reserve_vector_count).cuda()
                curr_labels = torch.arange(self.args.way * (session - 1), self.args.way * session).cuda() + self.args.base_class
                mask = ~torch.isin(target_labels, curr_labels)
                ...
                target_prototypes = torch.nn.Parameter(torch.cat((self.fc.get_classifier_weights(uptil=session+1).clone(), self.fc.unassigned_rv.clone())))
                target_prototypes.requires_grad = False
                target_labels = torch.arange(self.fc.reserve_vector_count).cuda()
            ...
            for epoch in tqdm_gen:
                ...
                for batch in jointloader:
                    ...
                    if self.args.skip_perturbation:
                        ...
                        pert, target_labels_ = perturb_targets_norm_count(target_prototypes.clone(), target_labels.clone(), bsz, 2, epsilon = self.args.perturb_epsilon_inc)
                        perturbed_t1, perturbed_t2 = pert
                    ...
                    # Cosine Pull Loss
                    pull_loss = torch.zeros(1)
                    if epoch >= self.args.pull_delay:
                        logits = self.fc(projections)
                        label_rep = label.repeat(2)
                        if self.args.apply_tbnce:
                            # compute the logit outputs given the rv for the projections and pass the logits to the criterion forward
