...
from .Network import MYNET, dot_reg_loss
from utils import *
...
def base_train(model, trainloader, optimizer, scheduler, epoch, args):
    ...
    for i, batch in enumerate(tqdm_gen, 1):
        ...
        enc_out, proj_out, proj = model(data)
        ...
        projection_loss = dot_reg_loss(proj, train_label, model.module.fixed_fc)

        # TODO: Add dot reg loss here
        # with torch.no_grad():
        #     proj_out_nograd = proj_out.detach()
        #     H_length = torch.clamp(torch.sqrt(torch.sum(proj_out_nograd ** 2, dim=1, keepdims=False)), 1e-8)
        # projection_loss = dot_loss(proj_out, train_label, cur_M, classifier, criterion, H_length, reg_lam=config.reg_lam)

        total_loss = projection_loss #+ fc_loss 
        ...
        tqdm_gen.set_description(
            'Session 0, epo {}, lrc={:.4f},total loss={:.4f} acc={:.4f}'.format(epoch, lrc, projection_loss.item(), proj_acc))
...
def test(model, testloader, epoch, args, session, base_sess = False, max_inference=False):
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(tqdm_gen, 1):
            ...
            # >>> Addition
            novelAcc, baseAcc = count_acc_(proj_out, test_label, test_class, args)
