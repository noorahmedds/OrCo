...
class MYNET(nn.Module):
    ...
    def update_fc(self, trainloader, testloader, class_list, session):
        ...
        if self.args.not_data_init:
            ...
            # TODO set the new_fc as the avg fc qr decomposed
            new_fc = self.update_fc_avg(data, label, class_list)
            # orth_vec, _ = np.linalg.qr(new_fc.detach().cpu().T)
            # new_fc = torch.tensor(orth_vec.T).float().cuda()

            # Initialise randomly if fc is too close to another
    ...
    def update_fc_avg(self,data,label,class_list):
        ...
        for class_index in class_list:
            ...
            # Weight an embedding with the reciprocal of the mean confidence of that prototype over the previous fc.
            # This will ensure that the embedding with the least confidence in the previous fc is promoted.
            # Sort of hard prototype mining
            linear_out = F.linear(embedding, self.fc.weight[:class_list.min()])
            lmean = linear_out.mean(axis = -1)

            # ----Simple init----
            weights = 1 - (lmean / lmean.sum())  
            weights = weights/weights.sum() 

            # ----Softmin init----
            # weights = F.softmin(lmean, dim = 0).detach()

            weighted_emb = weights.unsqueeze(1) * embedding
            
            proto = weighted_emb.sum(0)
            new_fc.append(proto)
            self.fc.weight.data[class_index]=proto
