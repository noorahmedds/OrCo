...
class FSCILTrainer(Trainer):
    ...
    def dino_pretraining(self):
        ...
        train_set, data_loader, testloader = get_dino_dataloader(self.args, args)


        print(f"Dino Data loaded")

        student = MYNET(self.args, mode="encoder", writer = self.writer)
        teacher = MYNET(self.args, mode="encoder", writer = self.writer)

        # student = nn.DataParallel(student, list(range(self.args.num_gpu)))
        # teacher = nn.DataParallel(teacher, list(range(self.args.num_gpu)))
        ...
        gpu_list = [int(x) for x in self.args.gpu.split(',')]
        if has_batchnorms(student):
            # student = nn.SyncBatchNorm.convert_sync_batchnorm(student)
            # teacher = nn.SyncBatchNorm.convert_sync_batchnorm(teacher)

            # we need DDP wrapper to have synchro batch norms working...
            # teacher = nn.parallel.DistributedDataParallel(teacher, device_ids=gpu_list)
            teacher = nn.DataParallel(teacher, list(range(self.args.num_gpu)))
        ...
        # student = nn.parallel.DistributedDataParallel(student, device_ids=gpu_list)
        student = nn.DataParallel(student, list(range(self.args.num_gpu)))
