...
class MYNET(nn.Module):
    ...
    def __init__(self, args, mode=None):
        ...
        if self.args.dataset == 'cub200':
            ...
            self.encoder = lcwof_net.resnet18(True, args)
            self.num_features = 512
    ...
    def load_my_state_dict(self, state_dict):
        ...
        for name, param in state_dict.items():
            ...
            if "fc" in name and own_state[name].shape != param.shape:
                own_state[name][:self.args.base_class, :].copy_(param)
            else:
    ...
    def update_fc(self, trainloader, testloader, class_list, session):
        ...
        if self.args.not_data_init:
            ...
            new_fc = self.update_fc_base_mean(class_list)
    ...
    def update_fc_base_mean(self,class_list, rand_count = 10):
        """
            Update new fc using the mean of 5 randomly sampled classes
        """
        new_fc=[]
        for class_index in class_list:
            rand_ix = torch.randperm(self.args.base_class)[:rand_count]
            proto=self.fc.weight[rand_ix, :].mean(axis = 0)
            new_fc.append(proto)
            self.fc.weight.data[class_index]=proto
        new_fc=torch.stack(new_fc,dim=0)
        return new_fc
        ...
        with torch.enable_grad():
            ...
            for epoch in tqdm_gen:
                ...
                if self.args.validation_metric == "hm":
                    # Validation
                    output = self.test_fc(concat_fc, testloader, epoch, session)
                    if best_hm is None or output['vhm'] > best_hm:
                        best_hm = output['vhm']
                        best_fc = new_fc.clone()
                    out_string = 'Session {}, current_loss {:.3f}, train_acc {:.3f}, current_vhm {:.3f}, best_vhm {:.3f}'\
                        .format(
                            session, 
                            total_loss,
                            float('%.3f' % (ta.item() * 100.0)),
                            float("%.3f" % (output['vhm'] * 100.0)),
                    ...
                    if best_acc is None or ta.item() > best_acc:
                        best_acc = ta.item()
                        best_fc = new_fc.clone()
                    out_string = 'Session {}, current_loss {:.3f}, train_acc {:.3f}'\
                        .format(session, total_loss,float('%.3f' % (ta.item() * 100.0)),)
    ...
    def update_fc_ft_joint(self, jointloader, testloader, class_list, session):
        ...
        with torch.enable_grad():
            ...
            for epoch in tqdm_gen:
                ...
                out_string = 'Session {} <Joint Training>, current_loss {:.3f}, best_loss {:.3f}, train_acc {:.3f}'\
                    .format(
                        session, 
                        total_loss,
                        best_loss if best_loss is not None else total_loss,
                        float('%.3f' % (ta.item() * 100.0))
                        )
                tqdm_gen.set_description(out_string)
    ...
    def test_fc(self, fc, testloader, epoch, session):
        ...
        vaNovel = Averager() # Averager for base classes only over joint set

        # test_fc = fc.clone().detach()
        vaBBase = Averager() # Averager for novel classes only        
        vaNNovel = Averager() # Averager for novel classes only
        ...
        with torch.no_grad():
            ...
            for batch in testloader:
                ...
                nnovelAcc, bbaseAcc = count_acc_(logits, test_label, test_class, self.args, sub_space="separate")
                vaBBase.add(bbaseAcc)
                vaNNovel.add(nnovelAcc)
            ...
            vaBBase = vaBBase.item()
            vaNNovel = vaNNovel.item()
