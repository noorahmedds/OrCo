...
class FSCILTrainer(Trainer):
    ...
    def ensure_dict(self):
        for name, param in self.best_model_dict.items():
            if "fc" in name:
                x = self.model.module.fc.weight.clone()
                x.data[:self.args.base_class, :].copy_(param.data)
                self.best_model_dict[name] = x
    ...
    def train(self):
        ...
        for session in range(args.start_session, args.sessions):
            ...
            if session == 0:  # load base class train img label
                ...
                tsl, tsa, tsaNovel, tsaBase, tsHm, tsAm, tcm, tcmNovel = test(self.model, testloader, 0, args, session)        # Perform the validation inside the classifier updates and save the best mode

                # Tensorboard logging
                self.writer.add_scalar('max_acc', tsa, session)
                self.writer.add_scalar('max_novel_acc', tsaNovel, session)
                self.writer.add_scalar('max_base_acc', tsaBase, session)
    ...
    def sessional_test(self):
        ...
        for session in range(args.start_session, 1):
            ...
            ret = self.model.load_state_dict(self.best_model_dict, strict = True)

            if session == 0:  # load base class train img label
                output = test(self.model, testloader, 0, args, session, base_sess = True)
                tsa = output["va"]
                tcm = output["cm"]
                print('Testing accuracy for base model: {:.3f}'.format(tsa * 100))
                tcm["cm"].savefig(os.path.join(self.args.save_path, f"cm_{session}.png")) 
            else:
                print("training session: [%d]" % session)
                self.model.module.mode = self.args.new_mode
                self.model.eval()
                ...
                torch.load(self.args.model_dir)['params']
