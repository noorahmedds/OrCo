...
def get_prototypes_for_session(trainset, transform, model, args, session, use_vector_variance = False, views=1, trainloader = None):
    ...
    with torch.no_grad():
        for v in range(views):
            for i, batch in enumerate(trainloader):
                ...
                image = images[0].cuda()    # Taking only the first view
                embedding = model.encode(image).detach()
...
def get_gaussian_for_session(trainset, transform, model, args, session, use_vector_variance = False, views=1, trainloader = None):
    # replace fc.weight with the embedding average of train data
    model = model.eval()
    ...
    gaussian_obj = {
        ...
        "numInstancesPerClass": numInstancesPerClass
    }
...
def augmentMultivariateData(sub_feature_matrix, sub_labels, feature_matrix, labels, mode="extrapolate"):
    ...
    for i in range(len(sub_feature_matrix)):
        ...
        if mode == "extrapolate":
            # Extrapolate the current vector using another random vector randomly
            new_row = ((sub_feature_matrix[i] - feature_matrix[random_index])*lam) + sub_feature_matrix[i]
...
def compute_aug_feature_matrix(feature_matrix, labels):
    ...
    for label in unique_labels:
        ...
        for i in indices_same_label:
            ...
            # TODO: Instead of averaging do extrapolation
