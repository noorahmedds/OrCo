...
class MYNET(nn.Module):
    ...
    def update_fc_ft_base(self, baseloader, testloader):
        ...
        if self.args.fine_tune_backbone_base:
            optimizer = torch.optim.SGD([
                ...
                {'params': self.encoder.parameters(), 'lr': 0.1}
            ...
            for epoch in tqdm_gen:
                ...
                for idx, batch in enumerate(baseloader):
                    ...
                    if self.args.mix_lam > 0:
                        x0 = images[:bsz, ...]
                        mixup_loss = mix_step(x0, f1, f2, model=self)
    ...
    def update_fc_ft_joint_supcon(self, jointloader, testloader, class_list, session, gaussian_obj):
        # Optimising only the projector
        ...
        # Guassian data
        gaussian_data, gaussian_labels = gaussian_utils.generateMutivariateData(gaussian_obj["prototypes"], gaussian_obj["prototypeLabels"], gaussian_obj["classVariances"], 5)
        gaussian_data_2, _ = gaussian_utils.generateMutivariateData(gaussian_obj["prototypes"], gaussian_obj["prototypeLabels"], gaussian_obj["classVariances"], 5)
        aug_gaus_data, aug_gaus_labels = gaussian_data, gaussian_labels = gaussian_utils.generateMutivariateData(gaussian_obj["prototypes"], gaussian_obj["prototypeLabels"], gaussian_obj["classVariances"], 200)
        ...
        with torch.enable_grad():
            ...
            for epoch in tqdm_gen:
                ...
                generated_data_2 = np.array_split(gaussian_data_2, total_iter)
                ...
                for i, batch in enumerate(jointloader):
                    images, label = batch
                    images = torch.cat([images[0], images[1]], dim=0)
                    if torch.cuda.is_available():
                        images = images.cuda(non_blocking=True)
                    ...
                    # Pass gaussian through the projector head
                    bsz_gaussian = generated_labels[i].shape[0]
                    g1 = gaussian_utils.augmentMultivariateData(generated_data[i], generated_labels[i], aug_gaus_data, aug_gaus_labels)
                    g2 = gaussian_utils.augmentMultivariateData(generated_data_2[i], generated_labels[i], aug_gaus_data, aug_gaus_labels)

                    gaussian_encodings = np.concatenate([g1, g2], axis = 0)
                    gaussian_encodings = torch.tensor(gaussian_encodings).to(torch.float).cuda()
                    projections_gaussian = self.projector(gaussian_encodings).detach()

                    # Normalise projections
                    projections = normalize(projections)
                    f1, f2 = torch.split(projections, [bsz, bsz], dim=0)
                    
                    # Normalise gaussian projection
                    projections_gaussian = normalize(projections_gaussian)
                    f1_gaus, f2_gaus = torch.split(projections_gaussian, [bsz_gaussian, bsz_gaussian], dim=0)

                    # Combine gauss and original image projections
                    label = torch.cat([label, torch.tensor(generated_labels[i]).cuda()]).to(torch.long)
                    bsz = label.shape[0]
                    projections = torch.cat([projections, projections_gaussian])
                    projections = normalize(projections)
                    f1, f2 = torch.split(projections, [bsz, bsz], dim=0)
