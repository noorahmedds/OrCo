...
def test_fixed(model, testloader, epoch, args, session):
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(tqdm_gen, 1):
            ...
            logits = model.module.fc(model.module.predictor(encodings))
            logits = logits[:, :test_class]
    ...
    return vl, va, novel_acc, base_acc, vaSession, cw_acc, novel_cw.item(), base_cw.item()
...
def extract_features(model, loader):
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(tqdm_gen, 1):
            ...
            _, feat = model.module.forward_metric(data)
