...
class MYNET(nn.Module):
    ...
    def update_fc_ft_base(self, baseloader, testloader, class_variance=None):
        ...
        # Using for combined pulling with supcon
        all_targets = torch.nn.Parameter(torch.cat((self.fc.get_classifier_weights(), self.fc.rv)))
        all_target_labels = torch.arange(self.fc.reserve_vector_count).cuda()
        ...
        # For class augmentation
        UL = self.fc.rv.shape[0]
        curr_batch_labels = torch.arange(self.args.base_class)
        curr_mix_pair = torch.arange(self.args.base_class)
        while (curr_mix_pair == curr_batch_labels).any(): curr_mix_pair = torch.randperm(self.args.base_class)
        curr_mix_pair = curr_mix_pair[:UL].cuda()
        pair_labels = (torch.arange(UL) + self.args.base_class).cuda()

        new_fc = nn.Linear(self.fc.rv.shape[1], self.fc.rv.shape[0], bias=False).cuda()
        new_fc.weight.data.copy_(self.fc.rv)
        self.fc.classifiers.append(new_fc)
        ...
        with torch.enable_grad():
            ...
            for epoch in tqdm_gen:
                ...
                for idx, batch in enumerate(baseloader):
                    ...
                    # Mixup the images that belong to the same pair
                    # Add the corresponding labels
                    if self.args.class_aug_base:
                        images, label = mixup_pairs(images, label, curr_mix_pair, self.args.base_class)
                    ...
                    if self.args.skip_perturbation_base:
                        f1_ = f1
                        f2_ = f2
                        label_ = label
                    else:
                        perturbed_t1 = perturb_targets_norm(all_targets, epsilon = 1e-2)
                        perturbed_t2 = perturb_targets_norm(all_targets, epsilon = 1e-2)
                        f1_ = torch.cat((f1, perturbed_t1), axis = 0)
                        f2_ = torch.cat((f2, perturbed_t2), axis = 0)
                        label_ = torch.cat((label, all_target_labels))

                        # Add targets as features with labels

                    features = torch.cat([f1_.unsqueeze(1), f2_.unsqueeze(1)], dim=1)
                    loss = self.args.sup_lam * sc_criterion(features, label_)
    ...
    def update_fc_ft_joint_supcon(self, jointloader, testloader, class_list, session):
        ...
        target_prototypes = torch.nn.Parameter(torch.cat((self.fc.get_classifier_weights(uptil=session)[self.args.base_class:], self.fc.rv)))
        ...
        # Using for combined pulling with supcon
        novel_assigned = self.fc.get_classifier_weights()[self.args.base_class:]
        all_targets = torch.nn.Parameter(torch.cat((novel_assigned, self.fc.rv)))
        all_target_labels = torch.arange(self.fc.reserve_vector_count - self.args.base_class).cuda()

        # For class augmentation
        UL = self.fc.rv.shape[0]
        curr_batch_labels = torch.arange(self.args.base_class+(self.args.way*session))
        curr_mix_pair = torch.arange(self.args.base_class+(self.args.way*session))
        while (curr_mix_pair == curr_batch_labels).any(): 
            curr_mix_pair = torch.randperm(self.args.base_class+(self.args.way*session))
        curr_mix_pair = curr_mix_pair[:UL].cuda()
        pair_labels = (torch.arange(UL) + (self.args.base_class+(self.args.way*session))).cuda()
        new_fc = nn.Linear(self.fc.rv.shape[1], self.fc.rv.shape[0], bias=False).cuda()
        new_fc.weight.data.copy_(self.fc.rv)
        self.fc.classifiers.append(new_fc)
        ...
        with torch.enable_grad():
            ...
            for epoch in tqdm_gen:
                ...
                for batch in jointloader:
                    ...
                    # Mixup the images that belong to the same pair
                    # Add the corresponding labels
                    if self.args.class_aug_inc:
                        images, label = mixup_pairs(images, label, curr_mix_pair, self.args.base_class+(self.args.way*session))
                    ...
                    if self.args.skip_perturbation_inc:
                        f1_ = f1
                        f2_ = f2
                        label_ = label
                    else:
                        perturbed_t1 = perturb_targets_norm(target_prototypes, epsilon = 1e-2)
                        perturbed_t2 = perturb_targets_norm(target_prototypes, epsilon = 1e-2)
                        f1_ = torch.cat((f1, perturbed_t1), axis = 0)
                        f2_ = torch.cat((f2, perturbed_t2), axis = 0)
                        label_ = torch.cat((label, target_labels))
                        
                    features = torch.cat([f1_.unsqueeze(1), f2_.unsqueeze(1)], dim=1)
                    sc_loss = self.args.sup_lam * sc_criterion(features, label_)
                    loss += sc_loss
