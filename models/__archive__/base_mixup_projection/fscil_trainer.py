    ...
    def train(self):
        ...
        for session in range(args.start_session, args.sessions):
            ...
            if session == 0:  # load base class train img label
                ...
                self.model.module.use_projector = True
                ...
                if not args.not_data_init:
                    ...
                    # Remove the projection layer
                    self.model.module.use_projector = False
                    
                    # Replacing the classifier with the encodings for the base classes
                    self.model = replace_base_fc(train_set, testloader.dataset.transform, self.model, args)
                    best_model_dir = os.path.join(args.save_path, 'session' + str(session) + '_max_acc.pth')
                    print('Remove projector & Replace the fc with average embedding, and save it to :%s' % best_model_dir)
                    self.best_model_dict = deepcopy(self.model.state_dict())
                    torch.save(dict(params=self.model.state_dict()), best_model_dir)

                    self.model.module.mode = 'avg_dot'      # Mode for testing if the base fc is replaced by the average feature vector
