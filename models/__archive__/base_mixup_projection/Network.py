...
class MYNET(nn.Module):
    ...
    def __init__(self, args, mode=None, writer=None):
        ...
        self.projector = projection_MLP(self.num_features, self.num_features, 2)
        self.fc = nn.Linear(self.num_features, self.args.num_classes, bias=False)       # Note the entire number of classes are already added
        # self.fc = nn.Linear(2048, self.args.num_classes, bias=False)       # Note the entire number of classes are already added

        # Initialise the fc layer
