...
def attention_meta_train(model, trainloader, optimizer, scheduler, epoch, args):
    # Using the episode shot and episode way, get a support and query set for the pseudo_base_class
    # for the psudo novel class use low shot low way.
    # Note we can do this using the same sampler
    # But using just a disjoint set between the novel and base class
    # pass them through the model encoder
    # Now you will have latent space vector
    # now before the fc layer
    # send the fc layer (optionally the prototypes just from the base and novel classes)
    # alongside the latent space vectors of the sampled data
    # The output should be of the fc layer size.
    # The fc layer and the latent space vectors multiply. 
    # Note the simulation of our task happens where we sample more pseudo base classes than novel classes. 
    # The self attention module should be able to attend to this bias in data and compensate for it within the fc layer
    # And provide a fc layer which attends to this problem.
    # Note maybe here it makes sense for us to introduce entirely new data for example using mixup
    # like the way alice does for our pseudo incremental class
    # This should give us the advantage that the pseudo novel classes can actually be
    # Final loss would be a simple cross entropy loss between the sampled class latent vector multiplied with the fully connected layer.
    # In this case we would have already base trained network. So lets assume our baseline 3.0

    # Sample psudo base classes
    tl = Averager()
    ta = Averager()
    ...
    # for a certain number of incremental sessions
    for i, batch in enumerate(tqdm_gen, 1):
        data, true_label = [_.cuda() for _ in batch]

        # Our data now contains the sampled data. We need to class augment them to create a pseudo novel class.
        # For each dataset we will use the original setting guides.
        
        # After class augmentation

        # Now during our actual testing we don't have acess to base prototypes. What should we do there?
        
        # What we should do is pass the latent vector and the fc layer just to the self attention module
        # To be able to directly emulate our novel session training, we should take the same classes everytime for mixup 
        # For some N sessions. 
        # Note the cross entropy is very similar to our novel training. In fact we use our base normalised cross entropy here as well with the added self attention module

        # TODO: Make only 5 classes. But ofcourse with the same label
        # So class a,b,c,d -> ab, ac, ad, bc, bd (Note same class twice is not allowed)
        # We need atleast N classes. We need to sample C classes
        # Such that C*C-1 / 2 >= N. For example for 5 way psudo incremental session
        # We need C = 4. a,b,c,d = ab, ac, ad, bc, bd, cd (6 permutations out of which we choose 5)
        # Now as for K shots we need to sample all the 
        # Now that we have the classes we need the samples
        # We sample the required classes from the data loader. 
        novel_data, labels = fusion_aug_one_image(data, true_label, 0, args)

        # TODO: Alternatively take only N classes here each. With 1 shot only

        logits = model.module.forward_attention(novel_data)
...
def attention_test(model, testloader, epoch, args, session, base_sess = False, max_inference=False):
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(tqdm_gen, 1):
            ...
            logits = model.module.forward_attention(data)
