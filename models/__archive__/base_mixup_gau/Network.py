...
class MYNET(nn.Module):
    ...
    def update_fc_joint(self, trainloader, testloader, class_list, session, generated):
        # if 'ft' in self.args.new_mode:  # further finetune
        #     # self.update_fc_ft(new_fc,data,label,session)
        return self.update_fc_ft_joint(trainloader, testloader, class_list, session, generated)

    def update_fc_ft_joint(self, trainloader, testloader, class_list, session, generated):
        """
        ...
        generated_data, generated_labels = generated
        ...
        # and then divide the generated_data into total_iter chunks
        # shuffle generated data
        shuffler = np.arange(generated_data.shape[0])
        np.random.shuffle(shuffler)
        generated_data = generated_data[shuffler]
        generated_labels = generated_labels[shuffler]
        
        # split equally
        total_iter = trainloader.__len__()
        generated_data = np.array_split(generated_data, total_iter)
        generated_labels = np.array_split(generated_labels, total_iter)
        ...
        with torch.enable_grad():
            ...
            for epoch in tqdm_gen:
                ...
                ta = Averager()
                # for i, batch in enumerate(tqdm_gen, 1):
                for i, (data, label) in enumerate(trainloader): # Note that the trainloader takes all availble images
                    if self.args.base_only:
