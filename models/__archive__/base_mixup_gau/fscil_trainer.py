...
class FSCILTrainer(Trainer):
    ...
    def train(self):
        ...
        # Prototypes
        prototypes = None
        prototypeLabels = None
        classVariances = None
        numInstancesPerClass = None
        ...
        for session in range(args.start_session, args.sessions):
            ...
            if session == 0:  # load base class train img label
                ...
                prototypes, prototypeLabels, classVariances, numInstancesPerClass = get_prototypes_for_session(train_set, testloader.dataset.transform, self.model, args, session, use_vector_variance=args.use_vector_variance)

            else:  # incremental learning sessions
                if prototypes is None:
                    base_set, _, _ = self.get_dataloader(0)
                    prototypes, prototypeLabels, classVariances, numInstancesPerClass = get_prototypes_for_session(base_set, testloader.dataset.transform, self.model, args, 0, use_vector_variance=args.use_vector_variance)
                ...
                # Generate mutlivariate data. 
                generated = generateMutivariateData(prototypes, prototypeLabels, classVariances, args.shot)

                # Fix train loader for joint session with batch size altered. Instead of using all the data use only part of the data. Because now the batch size is very high
                jointloader = torch.utils.data.DataLoader(dataset=train_set, batch_size=args.batch_size_joint, shuffle=True,
                                                  num_workers=args.num_workers, pin_memory=True)
                
                # The generate multi variate would now also produce prototypes for the previous novel classes
                self.model.module.update_fc_joint(jointloader, testloader, np.unique(train_set.targets), session, generated)
                ...
                self.trlog["max_hm"][session] = float('%.3f' % (hm(tsaNovel, tsaBase) * 100))
                self.trlog["max_am"][session] = float('%.3f' % (am(tsaNovel, tsaBase) * 100))
