...
class SRHead(nn.Module):
    def __init__(self, args, num_features):
        ...
        # Create an interpolating classifer which updates to the current classifier over a certain period
        self.interp_classifier = nn.Sequential(nn.Linear(self.num_features, self.args.base_class, bias=False))
        
        self.prototype_classifier = nn.Sequential(nn.Linear(self.num_features, self.args.base_class, bias=False))
    ...
    def get_assignment(self, cost, assignment_mode, prototypes):
        ...
        # Penalize cost when assignment causes high interference with already assigned targets 
    ...
    def assign_base_classifier(self, base_prototypes, base_cov=None):
        ...
        # Create interpolating linear layer
        self.prototype_classifier[0].weight.data = base_prototypes.clone()
        self.interp_classifier[0].weight.data = base_prototypes.clone()
    ...
    def assign_novel_classifier(self, new_prototypes, online = False, cov_list = None):  
        ...
        new_fc = nn.Linear(new_fc_tensor.shape[1], new_fc_tensor.shape[0], bias=False).cuda()
        new_fc.weight.data.copy_(new_prototypes)
        self.prototype_classifier.append(new_fc)

        new_fc = nn.Linear(new_fc_tensor.shape[1], new_fc_tensor.shape[0], bias=False).cuda()
        new_fc.weight.data.copy_(new_prototypes)
        self.interp_classifier.append(new_fc)
        self.inter_prog.append(0.0) # Indicating the progress for this classifier set
    ...
    def get_logits(self, encoding, session = 0):
        output = []
        for i, cls in enumerate(self.interp_classifier.children()):
    ...
    def update_interp_classifier(self, step=0.001):
        for i in range(len(self.inter_prog)):
            for param_proto, param_target, param_inter in zip(
                self.prototype_classifier[i].parameters(), self.classifiers[i].parameters(), self.interp_classifier[i].parameters()):
                param_inter.data = torch.lerp(param_proto.data, param_target.data, self.inter_prog[i])
            if self.inter_prog[i] < 1.0:
                self.inter_prog[i] += step
            else:
                self.inter_prog[i] = 1.0
...
class MYNET(nn.Module):
    ...
    def update_fc_ft_base(self, baseloader, testloader, class_variance=None):
        ...
        # Interp step
        interp_step = 1 / float(self.args.epochs_base/2.0)
        ...
        with torch.enable_grad():
            ...
            for epoch in tqdm_gen:
                ...
                for idx, batch in enumerate(baseloader):
                    ...
                    out_string = f"{self.fc.inter_prog}|||Epoch: {epoch}|[{idx}/{len(baseloader)}], Training Accuracy (Base): {ta.item()*100:.3f}, Validation Accuracy (Base): {best_acc*100:.3f}, simplex_loss: {sloss.item():.3f}"
                    tqdm_gen.set_description(out_string)

                # Interpolate the classifier to the the uniform targets. This should gradually lead to the correct answer and be less intrusive to other losses
                self.fc.update_interp_classifier(step=interp_step)
    ...
    def update_fc_ft_joint_supcon(self, jointloader, testloader, class_list, session):
        ...
        interp_step = 1 / float(self.args.epochs_joint/2)
