...
class MYNET(nn.Module):
    ...
    def update_fc_ft_base(self, baseloader, testloader, class_variance=None):
        ...
        AT = torch.nn.Parameter(assigned_targets)
            ...
            for epoch in tqdm_gen:
                ...
                for idx, batch in enumerate(baseloader):
                    ...
                    if self.args.skip_perturbation:
                        ...
                        pert_count = target_labels.shape[0]
                    features = torch.cat([f1_.unsqueeze(1), f2_.unsqueeze(1)], dim=1)
                    loss = self.args.sup_lam * sc_criterion(features, label_, pert_count = pert_count, AT=AT, at_labels = assigned_targets_label)
    ...
    def update_fc_ft_joint_supcon(self, jointloader, testloader, class_list, session):
        ...
        AT = torch.nn.Parameter(assigned_targets[self.args.base_class:])
        if self.args.novel_bias:
            novel_targets_label = torch.arange((self.args.way * session)) + novel_class_start
        else:
            novel_targets_label = torch.arange((self.args.way * session)) + self.args.base_class
        AT.requires_grad = False
        ...
        with torch.enable_grad():
            ...
            for epoch in tqdm_gen:
                ...
                for batch in jointloader:
                    ...
                    features = torch.cat([f1_.unsqueeze(1), f2_.unsqueeze(1)], dim=1)
                    sc_loss = self.args.sup_lam * sc_criterion(features, label_, pert_count = pert_count, AT=AT, at_labels = novel_targets_label)
                    loss += sc_loss
