...
class SupConLoss(nn.Module):
    ...
    def forward(self, features, labels=None, mask=None, margin = 0, hemisphere = False, pert_count = None, AT=None, at_labels = None):
        ...
        # Add cosine loss here
        if AT is not None:
            # Compute the mean vector for each logit pair
            # import pdb; pdb.set_trace()
            # import pdb; pdb.set_trace()
            mask = torch.eq(labels, at_labels.cuda()).float().to(device)
            anchor_dot_contrast = torch.div(torch.matmul(anchor_feature, AT.T), self.temperature)
            # for numerical stability
            logits_max, _ = torch.max(anchor_dot_contrast, dim=1, keepdim=True)
            logits = anchor_dot_contrast - logits_max.detach()  # logits now contain the dot product of the anchors with each feature
            ...
            # Cosine loss
            cos_logits = 1 - logits
            pull_loss = (cos_logits*mask).mean()
            loss += pull_loss

            # Log Prob Likelihood
            # exp_logits = torch.exp(logits)    # log Denominator
            # log_prob = logits - torch.log(exp_logits.sum(1, keepdim=True)) # Numerator - denominator. Remember to ln rule. Ln(e^x) = x so ln(numerator/denominator) = numerator - ln(denominator)
            # mean_log_prob_cos = (mask*logits).sum(1)
            # mask_sum = mask.sum(1)
            # mask_sum_mask = mask_sum > 0
            # mean_log_prob_cos = mean_log_prob_cos[mask_sum_mask] / mask_sum[mask_sum_mask]
            # pull_loss = - (self.temperature / self.base_temperature) * mean_log_prob_pos
            # loss += pull_loss.mean()

            # TODO:
            # Compute batch prototype and pull to target
            # Mean anchor features based on their class labels
            # create labels for this mean feature space
