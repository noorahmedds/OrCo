...
class MYNET(nn.Module):
    ...
    def __init__(self, args, mode=None, writer=None):
        ...
        # Classifier for the rotation
        self.rot_cardinality = 4
        # Note in original s2m2 the num_features for the linar rotation layer is 640. Here its 512, 64
        self.rotation_fc = nn.Linear(self.num_features, self.rot_cardinality)


    def forward_metric(self, x):
        if "rot" not in self.mode:
            x = self.encode(x)
        ...
        if 'cos' in self.mode:
            ...
            orig_img, rot_img = x
            orig_enc = self.encode(orig_img)
            rot_enc = self.encode(rot_img)

            ft_logits = self.fc(orig_enc)
            rot_logits = self.rotation_fc(rot_enc)
            return (rot_logits, ft_logits)
    ...
    def encode(self, x):
        ...
        # The average pool happens here. But know that there is no non linearity before the fully connected layer
    ...
    def forward_mixup(self, input, **kwargs):
        ...
        if 'cos' in self.mode:
            ...
            rot_logits = self.rotation_fc(x)
            ft_logits = self.fc(x)
            return rot_logits, ft_logits, y_a, y_b, lam
