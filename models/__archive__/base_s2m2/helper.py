...
def base_rot_train(model, trainloader, optimizer, scheduler, epoch, args, session = -1):
    tl = Averager()
    ta = Averager()
    model = model.train()
    ...
    for i, batch in enumerate(tqdm_gen, 1):
        # TODO
        orig_img, rot_img, rot_labels, cls_labels = [_.cuda() for _ in batch]
        data = (orig_img, rot_img)
        # TODO: Add original mixup here as well.

        # Rotation logits and class logits are separated
        rot_logits, cls_logits = model(data)
        rot_loss = F.cross_entropy(rot_logits, rot_labels)  # Don't need to divide because we don't have multiple outputs per image
        cls_loss = F.cross_entropy(cls_logits, cls_labels, label_smoothing=args.label_smoothing)

        rot_acc = count_acc(rot_logits, rot_labels)
        cls_acc = count_acc(cls_logits, cls_labels)

        # TODO: Add mm loss here as well
        if session == 0:
            # do the manifold mixup as well
            layer_mix = random.choice(args.mixup_layer_choice)
            mm_logits, targets_a, targets_b, lam = model.module.forward_mixup(data, targets = cls_labels, mixup = True, mixup_alpha = args.mixup_alpha, layer_mix = layer_mix)
            mm_loss = mixup_criterion(F.cross_entropy, mm_logits, targets_a, targets_b, lam)
            total_loss = mm_loss + (cls_loss + rot_loss)/2.0
            mm_acc = count_acc(mm_logits, cls_labels)
            total_acc = (rot_acc + cls_acc + mm_acc) / 3.0
        else:
            lam = 0.7
            total_loss = (1-lam)*rot_loss + lam*cls_loss
            total_acc = (rot_acc + cls_acc) / 2.0
            

        lrc = scheduler.get_last_lr()[0]
        tqdm_gen.set_description(
            '(S2M2) Session {}, epo {}, lrc={:.4f}, total loss={:.4f}(r:{:.3f},c:{:3f}), acc(rot, cls, total)=({:.3f},{:.3f},{:.3f})'.format(session, epoch, lrc, total_loss.item(), rot_loss.item(), cls_loss.item(), rot_acc, cls_acc, total_acc))
...
def rot_test(model, testloader, epoch, args, session):
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(tqdm_gen, 1):
            ...
            # rot_labels = rot_labels.flatten()
            # cls_labels = cls_labels.repeat_interleave(model.module.rot_cardinality)

            rot_logits, cls_logits = model(data)
            rot_loss = F.cross_entropy(rot_logits, rot_labels) # / model.module.rot_cardinality. Dont need to divide anymore because the number of rotations per image are on 1. 
            ...
            if session == 0:
                ...
                total_loss = rot_loss + cls_loss
                total_acc = (rot_acc + cls_acc) / 2
