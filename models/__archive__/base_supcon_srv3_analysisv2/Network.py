...
class MYNET(nn.Module):
                ...
                for idx, batch in enumerate(baseloader):
                    ...
                    if self.args.skip_perturbation:
                        ...
                        if self.args.perturb_dist == "uniform":
                            pert, target_labels_ = perturb_targets_norm_count(target_prototypes.clone(), target_labels.clone(), bsz, 2, epsilon = self.args.perturb_epsilon_base, offset=self.args.perturb_offset)
    ...
    def pull_loss(self, label_rep, novel_class_start, criterion, logits, tl, session):
        if self.args.pull_loss_mode == "default":
            ...
            if session_idx.numel() != 0:
                cos_loss += weight * self.criterion_forward(criterion, logits[session_idx, :], label_rep[session_idx])
        elif self.args.pull_loss_mode == "vanilla":
            cos_loss = self.criterion_forward(criterion, logits, label_rep)                            
            ...
            for epoch in tqdm_gen:
                ...
                for batch in jointloader:
                    ...
                    if self.args.skip_perturbation:
                        ...
                        if self.args.perturb_dist == "uniform":
                            pert, target_labels_ = perturb_targets_norm_count(target_prototypes.clone(), target_labels.clone(), bsz, 2, epsilon = self.args.perturb_epsilon_inc, offset=self.args.perturb_offset)
