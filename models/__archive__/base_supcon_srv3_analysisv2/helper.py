...
def get_new_fc(trainloader, model, args, class_list, views=1, return_embeddings=False, mode="encoder"):
    # replace fc.weight with the embedding average of train data
    model = model.eval()
...
def test_fixed(model, testloader, epoch, args, session, joint_prototypes):
    test_class = args.base_class + session * args.way
    model = model.eval()
    vl = Averager()
    va = Averager()
    ...
    # Save all_targets and all_probs and all_projections in a pickle file for each session
    # Saving : all_targets, all_probs, all_projections, args.base_class, session
    # Save this for our method
    art_save_dir = f"/BS/fscil/work/code/CEC-CVPR2021/figures/artefacts/{args.save_path_prefix}/"
    if not os.path.exists(art_save_dir):
        os.mkdir(art_save_dir)
    art_obj = {
        "all_targets": all_targets.cpu().numpy(),
        "all_probs": all_probs.cpu().numpy(),
        "all_projections": projection_features.cpu().numpy(),
        "session": session,
        "shot": args.shot,
        "way": args.way,
        "base_class": args.base_class,
        "fc": model.module.fc.get_classifier_weights().cpu().numpy(),
        "rv": model.module.fc.rv.cpu().numpy(),
        "joint_prototypes": joint_prototypes.cpu().numpy() if joint_prototypes is not None else None
    }
    with open(f'{art_save_dir}/testing_art_{session}.pickle', 'wb') as f:
        pickle.dump(art_obj, f)

    # Create confusion matrix
    cm = createConfusionMatrix( #plot_confusion_matrix(
        all_targets.cpu().numpy(), 
        torch.argmax(all_probs, axis = 1).cpu().numpy(),
        [str(i) for i in range(test_class)],
        hline_at = args.base_class,
        vline_at = args.base_class,
        session = session
    )
    save_path="/BS/fscil/work/code/CEC-CVPR2021/figures/cm"
    save_path = f"{save_path}/logit_map_{args.save_path_prefix}_{session}_CM.png"
    cm["cm"].savefig(save_path)

    # Create confusion matrix
    cm = createSessionalConfusionMatrix( #plot_confusion_matrix(
        all_targets.cpu().numpy(), 
        torch.argmax(all_probs, axis = 1).cpu().numpy(),
        [str(i) for i in range(test_class)],
        hline_at = args.base_class,
    ...
    save_path="/BS/fscil/work/code/CEC-CVPR2021/figures/cm_sessional"
    save_path = f"{save_path}/{args.save_path_prefix}_{session}_CM.png"
    cm["cm"].savefig(save_path)

    # Show TSNE projection of all_probs
    # Apply to a random set of classes
    # tsne_probs = TSNE(perplexity=100).fit_transform(all_probs.cpu().numpy())
    # plot_tsne(tsne_probs, all_targets.cpu().numpy(), args)

    # Compute average logits per class
    # target_to_avg_logits = {}
    # # class list = 
    # for i in range(args.base_class):
    #     class_logits = all_probs[all_targets == i].mean(0).cpu().numpy()
    #     target_to_avg_logits[i] = class_logits
    
    # if test_class > args.base_class:
    #     for i in range(args.base_class, test_class):
    #         class_logits = all_probs[all_targets == i].mean(0).cpu().numpy()
    #         target_to_avg_logits[i] = class_logits

    # draw_logit_map(target_to_avg_logits, args, subtitle=str(session), base_novel_avg=False)
    ...
    return vl, va, novel_acc, base_acc, vaSession, cw_acc, novel_cw.item(), base_cw.item(), cos_sims
...
def draw_logit_map(target_to_avg_logits, args, subtitle=None, save_path="/BS/fscil/work/code/CEC-CVPR2021/figures", base_novel_avg = False):
    save_path = save_path
    class_wise_logits = np.stack([i for i in target_to_avg_logits.values()])
    avg_labels = [i for i in target_to_avg_logits.keys()]
    
    if base_novel_avg:
        save_path = f"{save_path}/logit_map_{args.save_path_prefix}_{subtitle}_session_wise.png"
        # Average base logits
        base_logits = class_wise_logits[:args.base_class, ::].mean(0)
        novel_logits = class_wise_logits[args.base_class:, ::].mean(0)
        avg_labels = ["base", "novel"]
        # Generate colors for each row
        cmap = plt.cm.rainbow
        normalize = Normalize(vmin=0, vmax=class_wise_logits.shape[0] - 1)    
        fig, ax = plt.subplots()
        start_col = args.base_class
        end_col = class_wise_logits.shape[1]
        novel_color = 'lightcoral'
        base_color = 'lightblue'
        ax.axvspan(start_col, end_col + 0.5, color=novel_color, alpha=0.3)
        ax.axvspan(0, start_col, color=base_color, alpha=0.3)
        colors = ["red", "blue"]
        for i, row in enumerate([base_logits, novel_logits]):
            ax.plot(row, c=colors[i], linewidth = 0.7)
            row_text = f"{avg_labels[i]}"
            # ax.text(i, row[avg_labels[i]], row_text, va='bottom', c=row_color)
    else:
        save_path = f"{save_path}/logit_map_{args.save_path_prefix}_{subtitle}_class_wise.png"
        # Generate colors for each row
        ...
        # Plotting
        fig, ax = plt.subplots()
        # Set background color for a range of columns
        start_col = args.base_class
        end_col = class_wise_logits.shape[1]
        novel_color = 'lightcoral'
        base_color = 'lightblue'
        ...
        # Filter
        filtered_ix = [15, 62, 68, 73, 77, 81, 89, 93, 95]

        palette = np.array(sb.color_palette("hls", len(filtered_ix)))  #Choosing color palette 

        # Plotting
        colors = ["red", "blue", "magenta"]
        for i, row in enumerate(class_wise_logits):
            # if i == 0 or (i >= args.base_class and i % args.way == 0):
            # if i % args.way == 0:
            if i in filtered_ix:
                row_color = palette[filtered_ix.index(i)]
                ax.plot(row, c=row_color, linewidth = 0.7)
                # ax.text(avg_labels[i]-1, class_wise_logits.max(), avg_labels[i], va='bottom')
                row_text = f"{avg_labels[i]}"
                # if avg_labels[i] == row.argmax():
                #     row_text += " ✓"
                # else:
                #     row_text += " ✖"
                ax.text(avg_labels[i]-1, row[avg_labels[i]], row_text, va='bottom', c=row_color)
        
        
    legend_elements = [Patch(facecolor=base_color,
                             label='Base Logits', alpha = 0.3),
                       Patch(facecolor=novel_color,
                             label='Incremental Logits', alpha = 0.3)
                      ]
    ax.legend(handles=legend_elements, loc='best')

    plt.xlabel('Classes')
    plt.ylabel('Logit Value')
    plt.suptitle('Logit Map')
    if subtitle is not None:
        plt.title(f"Session {subtitle}", fontsize=10)
    fig.savefig(save_path)
    plt.close(fig)
    # plt.show()


def plot_tsne(x, colors, args, save_path="/BS/fscil/work/code/CEC-CVPR2021/figures/tsne"):
    # Visualise only 5 randomly chosen classes in the base session
    # and 1 class in each incremental session
    filter_ixs = np.array([0, 15, 21, 33, 51, 62, 68, 73, 77, 81, 89, 93, 95])
    palette = np.array(sb.color_palette("husl", np.unique(colors).shape[0]))  #Choosing color palette 

    mask = np.in1d(colors, filter_ixs)
    colors = colors[mask]
    x = x[mask]
    ...
    save_path = f"{save_path}/logit_map_{args.save_path_prefix}_TSNE.png"
    f.savefig(save_path)
    plt.close(f)