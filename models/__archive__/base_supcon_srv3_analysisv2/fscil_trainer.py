...
class FSCILTrainer(Trainer):
    ...
    def train(self):
        ...
        if args.sup_con_pretrain:
            ...
            out = test_fixed(self.model, base_testloader, 0, args, 0, None)
            best_va = out[1]
            cw_acc = out[5]
            cos_sims = out[8]
        ...
        for session in range(args.start_session, args.end_session):
            ...
            # Get best prototypes for joint set
            # test_class = torch.arange(self.args.base_class + (session*self.args.way))
            # joint_prototypes = joint_prototypes = get_new_fc(jointloader, self.model, self.args, test_class)
            joint_prototypes = None

            # Compute accuracy and KNN score for pretrain session
            tsl, tsa, tsaNovel, tsaBase, vaSession, cw_acc, novel_cw, base_cw, cos_sims = test_fixed(self.model, testloader, 0, args, session, joint_prototypes)
            ...
            out_string = 'Session {}, test Acc {:.3f}, test_novel_acc {:.3f}, \
                test_base_acc {:.3f}, hm {:.3f}, am {:.3f}, cw_acc {:.3f}, hm_cw {:.3f}\
                    cos_sims_inter {:.3f}, cos_sims_intra {:.3f}'\
                .format(
                    session, 
