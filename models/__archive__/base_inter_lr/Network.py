...
class MYNET(nn.Module):
    ...
    def update_fc_ft_joint(self, jointloader, testloader, class_list, session):
        ...
        inter_fc.data.copy_(self.fc.weight[self.args.base_class:self.args.base_class + (self.args.way * (session-1)), :])

        inter_lr = self.args.lr_new / 10.0

        optimized_parameters = [{'params':base_fc}, {'params':new_fc}, {'params':inter_fc, 'lr':inter_lr}]
        optimizer = self.get_optimizer_new(optimized_parameters)
