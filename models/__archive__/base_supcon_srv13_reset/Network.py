...
class SRHead(nn.Module):
    ...
    def assign_novel_classifier(self, new_prototypes, online = False, cov_list = None):  
        ...
        session_count = new_fc_tensor.shape[0] / self.args.way
        import pdb; pdb.set_trace()
        for i in range(session_count):
            session_start = self.way * i
            session_end = self.way * i+1
            if i+1 < len(self.classifiers):
                self.classifiers[i+1].data.copy_(new_fc_tensor[session_start:session_end, :].cuda())
            else:
                new_fc = nn.Linear(new_fc_tensor.shape[0], self.way, bias=False).cuda()
                new_fc.weight.data.copy_(new_fc_tensor[session_start:session_end, :])
...
class MYNET(nn.Module):
    ...
    def update_fc(self, trainloader, testloader, class_list, session, mode="encoder"):
        if not self.args.online_assignment: # Skip assignment if online assignment
            ...
            novel_classes_idx = torch.argwhere(labels >= self.args.base_class).flatten()
            datas = datas[novel_classes_idx]    
            labels = labels[novel_classes_idx]
            new_prototypes, cov_list = self.get_class_avg(datas, labels, class_list)
