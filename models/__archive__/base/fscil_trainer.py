        ...
        for session in range(args.start_session, args.sessions):
            ...
            if session == 0:  # load base class train img label
                ...
                for epoch in range(args.epochs_base):
                    ...
                    self.writer.add_scalars(f'base/', {
                        "train_accuracy":ta,
                        "test_accuracy": tsa
                    }, epoch)
                ...
                if not args.not_data_init:
                    ...
                    self.model.module.mode = 'avg_cos'      # Check if this is necessary. Having a disconect between the testing scheme in during base session and during the inference scheme which should create some unnecessary biases. Remove this if necessry.
                    tsl, tsa = (test(self.model, testloader, 0, args, session, base_sess = True))[:2]
                    if (tsa * 100) >= self.trlog['max_acc'][session]:
                        self.trlog['max_acc'][session] = float('%.3f' % (tsa * 100))
                        self.trlog['max_base_acc'][session] = float('%.3f' % (tsa * 100))
    ...
    def sessional_test(self):
        ...
        for session in range(args.start_session, args.sessions):
            ...
            if session == 0:  # load base class train img label
                ...
                if args.max_inference:
                    tsa, tsHm, base_t, novel_t = grid_search_test(self.model, testloader, 0, args, session)
                    print("HM: ", tsHm, ", Base Temp: ", base_t, ", Novel Temp: ", novel_t)
                    continue
                else:
                ...
                # Save tcm and tcmNovel into the files
