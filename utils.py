...
def mix_step(x0, z0, z1, mix_temperature = 0.05, model = None, detach_f = True):
    ...
    if model is not None:
        mix_projections = model.encode(mix_data, detach_f=detach_f) # Getting projections and comapring against z0 and z1
    else:
    ...
    # Apply softmax. We want to maximize the lambda mix between projections as 
...
def plot_grad_flow(named_parameters):
    ...
    for n, p in named_parameters:
        if(p.requires_grad) and ("bias" not in n and "fc" not in n) and (p.grad is not None):
            layers.append(n)
...
# import math
# def perturb_targets_norm(tensor, angle_degrees=10, epsilon=1e-6):
#     epsilon=1e-6
#     # Convert angle to radians
#     angle_radians = math.radians(angle_degrees)
#     # Generate random unit vectors for the perturbation directions
#     perturbation_directions = torch.randn_like(tensor)
#     perturbation_directions /= torch.norm(perturbation_directions, dim=1, keepdim=True)
#     # Calculate perturbation magnitudes based on angle
#     perturbation_magnitudes = torch.tan(angle_radians * torch.ones(tensor.shape[0])) * 1e-1
#     # Perturb each vector in the tensor
#     perturbed_tensor = tensor + perturbation_magnitudes.view(-1, 1).cuda() * perturbation_directions.cuda()
#     # Renormalize each vector in the tensor
#     perturbed_tensor /= torch.norm(perturbed_tensor, dim=1, keepdim=True)
...
if __name__ == "__main__":
    ...
    model_dict1 = torch.load("checkpoint/mini_imagenet/lcwof/exp10-15W-1S-15Q-50Epi-L15W-L1SEpo_500-Lr1_0.100000-Lrg_0.10000-Step_40-Gam_0.10-T_16.00/incremental_session_1_max_acc.pth")['params']
    model_dict8 = torch.load("checkpoint/mini_imagenet/lcwof/exp10-15W-1S-15Q-50Epi-L15W-L1SEpo_500-Lr1_0.100000-Lrg_0.10000-Step_40-Gam_0.10-T_16.00/incremental_session_8_max_acc.pth")['params']
