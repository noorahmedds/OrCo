...
def get_command_line_parser():
    ...
    parser.add_argument('-train_inter', action="store_true", help="Train intermediate fc as well during joint session")
    ...
    # about mixup
    parser.add_argument('-mixup_alpha', type=float, default=20.0, help = "Alhpa value for the beta distribution used to sample the mixing lambda, sample lambda applies to all the sessions")
    parser.add_argument('-mixup_base', action='store_true', help='Use mixup during base session on the input image')
    parser.add_argument('-mixup_novel', action='store_true', help='Use mixup during novel session')
    parser.add_argument('-mixup_joint', action='store_true', help='Use mixup during joint session')
    parser.add_argument('-mixup_hidden', action='store_true', help='Mixup during the base session in the latent space')
    parser.add_argument('-mixup_layer_choice', default='1,2,3,4')

    # about pre-training
    parser.add_argument('-epochs_base', type=int, default=100)
    parser.add_argument('-epochs_new', type=int, default=100)
    parser.add_argument('-epochs_novel', type=int, default=60)
    parser.add_argument('-epochs_joint', type=int, default=100)
    parser.add_argument('-lr_base', type=float, default=0.1) # 3e-4
    ...
    parser.add_argument('-batch_size_sup_con', type=int, default=128)                                                                             # 128
    ...
    parser.add_argument('-base_mode', type=str, default='ft_dot', # CEC -> ft_cos
                        choices=['ft_dot', 'ft_cos', 'ft_l2', "ft_dot_freeze"]) # ft_dot means using linear classifier, ft_cos means using cosine classifier
    ...
    parser.add_argument('-pull_criterion_base', type=str, default='xent', # CEC -> ft_cos
                        choices=["xent", "cosine", "xent+cosine", "cosine-squared", "none"]) # ft_dot means using linear classifier, ft_cos means using cosine classifier
    parser.add_argument('-pull_criterion_novel', type=str, default='xent', # CEC -> ft_cos
                        choices=["xent", "cosine", "xent+cosine", "cosine-squared", "none"]) # ft_dot means using linear classifier, ft_cos means using cosine classifier
    ...
    parser.add_argument("-proj_type", type=str, default="proj",
                        choices=["linear", "proj", "proj_alice", "proj_ncfscil", "proj_mlp"])

    parser.add_argument('-sup_lam', type=float, default=1)
    parser.add_argument('-cos_lam', type=float, default=1)
    parser.add_argument('-simplex_lam', type=float, default=0)
    parser.add_argument('-simplex_lam_inc', type=float, default=0)
    ...
    parser.add_argument("-pull_loss_mode", type=str, default="default",
                        choices=["default", "weighted", "curr_session", "vanilla"])
    ...
    parser.add_argument('-init_sess_w_base_proj', action="store_true") 
